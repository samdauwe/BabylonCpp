...
struct BABYLON_SHARED_EXPORT IPhysicsEnginePlugin {
  ...
  virtual void executeStep(float delta,
                           const std::vector<std::unique_ptr<PhysicsImpostor>>& impostors)
    = 0; // not forgetting pre and post events
  virtual void applyImpulse(const PhysicsImpostor& impostor, const Vector3& force,
  ...
  virtual void setTransformationFromPhysicsBody(const PhysicsImpostor& impostor) = 0;
  ...
  virtual void setBodyFriction(const PhysicsImpostor& impostor, float friction)       = 0;
  virtual float getBodyRestitution(const PhysicsImpostor& impostor)                   = 0;
  virtual void setBodyRestitution(const PhysicsImpostor& impostor, float restitution) = 0;
  virtual float getBodyPressure(const PhysicsImpostor& impostor)                      = 0;
  virtual void setBodyPressure(const PhysicsImpostor& impostor, float pressure)       = 0;
  virtual float getBodyStiffness(const PhysicsImpostor& impostor)                     = 0;
  virtual void setBodyStiffness(const PhysicsImpostor& impostor, float stiffness)     = 0;
  virtual size_t getBodyVelocityIterations(const PhysicsImpostor& impostor)           = 0;
  virtual void setBodyVelocityIterations(const PhysicsImpostor& impostor, size_t velocityIterations)
    = 0;
  virtual size_t getBodyPositionIterations(const PhysicsImpostor& impostor) = 0;
  virtual void setBodyPositionIterations(const PhysicsImpostor& impostor, size_t positionIterations)
    = 0;
  virtual void appendAnchor(const PhysicsImpostor& impostor,
                            const PhysicsImpostorPtr& otherImpostor, int width, int height,
                            float influence, bool noCollisionBetweenLinkedBodies)
    = 0;
  virtual void appendHook(const PhysicsImpostor& impostor, const PhysicsImpostorPtr& otherImpostor,
                          float length, float influence, bool noCollisionBetweenLinkedBodies)
    = 0;
  ...
  // Joint Update
  virtual void updateDistanceJoint(DistanceJoint* joint, float maxDistance, float minDistance) = 0;
  virtual void setMotor(IMotorEnabledJoint* joint, float speed, float maxForce,
                        unsigned int motorIndex = 0)
    = 0;
  virtual void setLimit(IMotorEnabledJoint* joint, float upperLimit, float lowerLimit,
                        unsigned int motorIndex = 0)
    = 0;
  virtual float getRadius(const PhysicsImpostor& impostor)                               = 0;
  virtual void getBoxSizeToRef(const PhysicsImpostor& impostor, Vector3& result)         = 0;
  virtual void syncMeshWithImpostor(AbstractMesh* mesh, const PhysicsImpostor& impostor) = 0;
