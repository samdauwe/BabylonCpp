...
FWD_STRUCT_SPTR(ImageProcessingConfigurationDefines)
...
public:
  ...
  /**
   * Multiply current color with particle color then add current color and particle color multiplied
   * by particle’s alpha
   */
...
protected:
  ...
  /**
   * @brief Gets whether an animation sprite sheet is enabled or not on the particle system.
   */
  [[nodiscard]] bool get_isAnimationSheetEnabled() const override;

  /**
   * @brief Sets whether an animation sprite sheet is enabled or not on the particle system.
   */
  ...
  /**
   * @brief Random direction of each particle after it has been emitted, between direction1 and
   * direction2 vectors. This only works when particleEmitterTyps is a BoxParticleEmitter.
   */
  ...
  /**
   * @brief Minimum box point around our emitter. Our emitter is the center of particles source, but
   * if you want your particles to emit from more than one point, then you can tell it to do so.
  ...
  /**
   * @brief Maximum box point around our emitter. Our emitter is the center of particles source, but
   * if you want your particles to emit from more than one point, then you can tell it to do so.
   * This only works when particleEmitterTyps is a BoxParticleEmitter.
   */
  ...
  /**
   * @brief Gets or sets a boolean indicating if the particles must be rendered as billboard or
   * aligned with the direction.
   */
  [[nodiscard]] bool get_isBillboardBased() const override;

  /**
   * @brief Sets a boolean indicating if the particles must be rendered as billboard or aligned with
   * the direction.
   */
