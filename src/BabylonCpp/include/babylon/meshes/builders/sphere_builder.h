...
struct BABYLON_SHARED_EXPORT SphereBuilder {
   ...
   * * You can set some different sphere dimensions, for instance to build an ellipsoid, by using
   * the parameters `diameterX`, `diameterY` and `diameterZ` (all by default have the same value of
   * `diameter`)
   * * The parameter `segments` sets the sphere number of horizontal stripes (positive integer,
   * default 32)
   * * You can create an unclosed sphere with the parameter `arc` (positive float, default 1),
   * valued between 0 and 1, what is the ratio of the circumference (latitude) : 2 x PI x ratio
   * * You can create an unclosed sphere on its height with the parameter `slice` (positive float,
   * default1), valued between 0 and 1, what is the height ratio (longitude)
