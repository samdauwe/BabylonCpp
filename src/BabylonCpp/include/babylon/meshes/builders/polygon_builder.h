...
struct BABYLON_SHARED_EXPORT PolygonBuilder {
   ...
   * The polygon's shape will depend on the input parameters and is constructed parallel to a ground
   * mesh
   * * The parameter `shape` is a required array of successive Vector3 representing the corners of
   * the polygon in th XoZ plane, that is y = 0 for all vectors
   * * You can set the mesh side orientation with the values : Mesh.FRONTSIDE (default),
   ...
   * * You can set different colors and different images to the top, bottom and extruded side by
   * using the parameters `faceColors` (an array of 3 Color3 elements) and `faceUV` (an array of 3
   * Vector4 elements)
