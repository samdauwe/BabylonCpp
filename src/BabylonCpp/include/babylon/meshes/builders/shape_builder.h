...
public:
  /**
   * @brief Creates an extruded shape mesh. The extrusion is a parametric shape. It has no
   ...
   * to be extruded in its local space : the shape must be designed in the xOy plane and will be
   * extruded along the Z axis.
   * * The parameter `path` is a required array of successive Vector3. This is the axis curve the
   * shape is extruded along.
   * * The parameter `rotation` (float, default 0 radians) is the angle value to rotate the shape
   * each step (each path point), from the former step (so rotation added each step) along the
   * curve.
   ...
   * * The optional parameter `instance` is an instance of an existing ExtrudedShape object to be
   ...
   * * Remember you can only change the shape or path point positions, not their number when
   * updating an extruded shape.
   ...
   * The custom extrusion is a parametric shape. It has no predefined shape. Its final shape will
   ...
   * * The parameter `path` is a required array of successive Vector3. This is the axis curve the
   * shape is extruded along.
   * * The parameter `rotationFunction` (JS function) is a custom Javascript function called on each
   * path point. This function is passed the position i of the point in the path and the distance of
   * this point from the begining of the path
   * * It must returns a float value that will be the rotation in radians applied to the shape on
   * each path point.
   * * The parameter `scaleFunction` (JS function) is a custom Javascript function called on each
   * path point. This function is passed the position i of the point in the path and the distance of
   * this point from the begining of the path
   * * It must returns a float value that will be the scale value applied to the shape on each path
   * point
   * * The parameter `ribbonClosePath` (boolean, default false) forces the extrusion underlying
   * ribbon to close all the paths in its `pathArray`
   * * The parameter `ribbonCloseArray` (boolean, default false) forces the extrusion underlying
   * ribbon to close its `pathArray`
   * * The parameter `cap` sets the way the extruded shape is capped. Possible values :
   * BABYLON.Mesh.NO_CAP (default), BABYLON.Mesh.CAP_START, BABYLON.Mesh.CAP_END,
   * BABYLON.Mesh.CAP_ALL
   * * The optional parameter `instance` is an instance of an existing ExtrudedShape object to be
