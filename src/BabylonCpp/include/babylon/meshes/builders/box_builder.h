...
struct BABYLON_SHARED_EXPORT BoxBuilder {
   ...
   * * You can set some different box dimensions by using the parameters `width`, `height` and
   * `depth` (all by default have the same value of `size`)
   * * You can set different colors and different images to each box side by using the parameters
   * `faceColors` (an array of 6 Color3 elements) and `faceUV` (an array of 6 Vector4 elements)
   ...
   */
  static MeshPtr CreateBox(const std::string& name, BoxOptions& options, Scene* scene = nullptr);
