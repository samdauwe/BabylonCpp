...
struct BABYLON_SHARED_EXPORT TiledBoxBuilder {
   ...
   * faceTiles sets the pattern, tile size and number of tiles for a face     * * You can set
   * different colors and different images to each box side by using the parameters `faceColors` (an
   * array of 6 Color3 elements) and `faceUV` (an array of 6 Vector4 elements)
   ...
   */
  static MeshPtr CreateTiledBox(const std::string& name, TiledBoxOptions& options,
                                Scene* scene = nullptr);
