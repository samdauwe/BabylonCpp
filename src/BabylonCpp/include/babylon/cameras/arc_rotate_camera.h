 ...
 *
 * This camera always points towards a given target position and can be rotated around that target
 * with the target as the centre of rotation. It can be controlled with cursors and mouse, or with
 * touch events. Think of this camera as one orbiting its target position, or more imaginatively as
...
public:
  ...
  /**
   * @brief Sets the Y-up to camera up-vector rotation matrix, and the up-vector to Y-up rotation
   * matrix.
   */
  void setMatUp();
  ...
  /**
   * @brief Stores the current state of the camera (alpha, beta, radius and target).
  ...
  /**
   * @brief Use a position to define the current camera related information like alpha, beta and
   * radius.
   ...
   * @param toBoundingCenter In case of a mesh target, defines wether to target the mesh position or
   * its bounding information center
   * @param allowSamePosition If false, prevents reapplying the new computed position if it is
   * identical to the current one (optim)
   */
  ...
  /**
   * @brief Focus on a mesh or a bounding box. This adapts the target and maxRadius if necessary but
   * does not update the current radius. The target will be changed but the radius
...
protected:
  ...
  /**
   * @brief The vector the camera should consider as up. (default is Vector3(0, 1, 0) as returned by
...
public:
  ...
  /**
   * Minimum allowed distance of the camera to the target (The camera can not get closer). This can
   * help limiting how the Camera is able to move in the scene.
   */
  std::optional<float> lowerRadiusLimit;

  /**
   * Maximum allowed distance of the camera to the target (The camera can not get further). This can
   * help limiting how the Camera is able to move in the scene.
   */
   ...
   * Basically if your fingers moves away from more than this distance you will be considered in
   * pinch mode.
   */
