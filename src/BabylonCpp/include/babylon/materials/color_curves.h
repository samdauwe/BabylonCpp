...
public:
  ...
  /**
   * @brief Prepare the list of uniforms associated with the ColorCurves effects.
   * @param uniformsList The list of uniforms used in the effect
   */
  static void PrepareUniforms(std::vector<std::string>& uniformsList);

  /**
   * @brief Returns color grading data based on a hue, density, saturation and exposure value.
  ...
  void getColorGradingDataToRef(float hue, float density, float saturation, float exposure,
                                Color4& result);
  /**
   * @brief Takes an input slider value and returns an adjusted value that provides extra control
   * near the centre.
  ...
  /**
   * @brief Returns an RGBA Color4 based on Hue, Saturation and Brightness (also referred to as
   * value, HSV).
...
protected:
   ...
   * The density value is in range [-100,+100] where 0 means the color filter has no effect and +100
