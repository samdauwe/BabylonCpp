...
/**
 * @brief Octrees are a really powerful data structure that can quickly select entities based on
 * space coordinates.
...
public:
   ...
   * (default: 64)
   * @param maxDepth defines the maximum depth (sub-levels) for your octree. Default value is 2,
   * which means 8 8 8 = 512 blocks :) (This parameter takes precedence over capacity.)
   */
  Octree();
  Octree(const std::function<void(T& entry, OctreeBlock<T>& block)>& creationFunc,
   ...
   */
  // clang-format on
  void update(const Vector3& worldMin, const Vector3& worldMax, std::vector<T>& entries);
   ...
   */
  std::vector<T>& select(const std::array<Plane, 6>& frustumPlanes, bool allowDuplicate = true);

  /**
   * @brief Test if the octree intersect with the given bounding sphere and if yes, then add its
   * content to the selection array.
  ...
  /**
   * @brief Test if the octree intersect with the given ray and if yes, then add its content to
   * resulting array.
   ...
   */
  static void CreationFuncForMeshes(AbstractMesh* entry, OctreeBlock<AbstractMesh*>& block);

  /**
   * @brief Adds a submesh into the octree block if it intersects the block.
   */
  static void CreationFuncForSubMeshes(SubMesh* entry, OctreeBlock<SubMesh*>& block);
...
public:
  ...
  /**
   * Defines the maximum depth (sub-levels) for your octree. Default value is 2, which means 8 8 8 =
   * 512 blocks :) (This parameter takes precedence over capacity.)
   */
  std::size_t maxDepth;
