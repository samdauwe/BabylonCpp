...
struct ICullable {
   ...
   */
  virtual bool isInFrustum(const std::array<Plane, 6>& frustumPlanes, unsigned int strategy = 0)
    = 0;
   ...
   * @returns true if the object is in frustum otherwise false
   */
  virtual bool isCompletelyInFrustum(const std::array<Plane, 6>& frustumPlanes) = 0;
