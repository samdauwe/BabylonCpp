#ifndef BABYLON_CORE_COMPILE_TIME_RNG_H
#define BABYLON_CORE_COMPILE_TIME_RNG_H
...
template <typename T, T EntropySeed, T Hash,
...
struct hash_impl {
  ...
  using result = typename hash_impl<T, EntropySeed,
                                    static_cast<T>((Hash * EntropySeed) ^ Hash
                                                   ^ TEntropy<Index>::value),
                                    TEntropy, Index + 1>::result;
};
...
template <typename T, template <std::size_t> class TEntropy, T EntropySeed,
          T Seed>
...
template <typename T>
using entropy = std::integral_constant<T, static_cast<T>(__cplusplus ^ __LINE__
 ...
 *  using rng = BABYLON_COMPILE_TIME_RNG<unsigned>;
 *
 ...
 */
template <
  typename T = std::size_t, T PreSeed = 0,
  T EntropySeed
  = detail::compile_time_rng_impl::default_compile_time_rng_prime<T>::value>
 ...
 *  using rng1 = BABYLON_COMPILE_TIME_RNG<>;
 *
 *  auto const n = rng1::get<>::value;
 *
 *  // yields `compile_time_rng<unsigned, *implementation defined*>`
 *  using rng2 = BABYLON_COMPILE_TIME_RNG<unsigned>;
 *
 *  auto const x = rng2::get<>::value;
 *
 * @author: Marcelo Juchem <marcelo@fb.com>
 */
#define BABYLON_COMPILE_TIME_RNG                                               \
  ::BABYLON::detail::compile_time_rng_impl::helper<__LINE__>::bind

} // end of namespace BABYLON

#endif // BABYLON_CORE_COMPILE_TIME_RNG_H
