...
struct LogMessageHandler {
  ...
  std::unordered_map<unsigned int, std::vector<LogMessageListener*>> _logMessageListeners;
  unsigned int _minLevel, _maxLevel;
};

/**
 * @brief Logger used througouht the application to allow configuration of the log level required
 * for the messages.
 */
...
public:
  ...
  static LogMessage CreateMessage(unsigned int level, std::string context, char const* file,
                                  int lineNumber, char const* func, char const* prettyFunc);
  ...
  void registerLogMessageListener(unsigned int level, LogMessageListener& logMsgListener);
  void unregisterLogMessageListener(const LogMessageListener& logMsgListener);
  void unregisterLogMessageListener(unsigned int level, const LogMessageListener& logMsgListener);
...
#define BABYLON_LOG_MSG(level, context, ...)                                                       \
  if (BABYLON::LoggerInstance().takes(level)) {                                                    \
    ...
    _ctx << context;                                                                               \
    BABYLON::LogMessage _logMessage = BABYLON::Logger::CreateMessage(                              \
      level, _ctx.str(), __FILE__, __LINE__, __FUNCTION__, __PRETTY_FUNCTION__);                   \
...
#define BABYLON_LOGF_ERROR(context, printf_like_message, ...)                                      \
  BABYLON_LOGF_MSG(BABYLON::LogLevels::LEVEL_ERROR, context, printf_like_message, __VA_ARGS__)
#define BABYLON_LOGF_WARN(context, printf_like_message, ...)                                       \
  BABYLON_LOGF_MSG(BABYLON::LogLevels::LEVEL_WARN, context, printf_like_message, __VA_ARGS__)
#define BABYLON_LOGF_INFO(context, printf_like_message, ...)                                       \
  BABYLON_LOGF_MSG(BABYLON::LogLevels::LEVEL_INFO, context, printf_like_message, __VA_ARGS__)
#define BABYLON_LOGF_DEBUG(context, printf_like_message, ...)                                      \
  BABYLON_LOGF_MSG(BABYLON::LogLevels::LEVEL_DEBUG, context, printf_like_message, __VA_ARGS__)
