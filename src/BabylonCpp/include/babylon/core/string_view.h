...
public:
  ...
  string_view(const char* str) : str_(str), len_(std::strlen(str))
  {
  }
  string_view(const char* str, size_t len) : str_(str), len_(len)
  {
  }
  string_view(const std::string& str) : str_(str.data()), len_(str.size())
  {
  }
...
#endif // end of BABYLON_CORE_STRING_VIEW_H
