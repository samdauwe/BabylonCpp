...
{
  Node::AddNodeConstructor("StereoscopicUniversalCamera", [](const std::string& iName, Scene* scene,
    ...
    return StereoscopicUniversalCamera::New(iName, Vector3::Zero(), interaxialDistance,
                                            isStereoscopicSideBySide, scene);
  });
  StereoscopicUniversalCamera::NodeConstructorAdded = true;
}

StereoscopicUniversalCamera::StereoscopicUniversalCamera(const std::string& iName,
