...
{
  Node::AddNodeConstructor("StereoscopicFreeCamera", [](const std::string& iName, Scene* scene,
    ...
    return StereoscopicFreeCamera::New(iName, Vector3::Zero(), interaxialDistance,
                                       isStereoscopicSideBySide, scene);
  });
  StereoscopicFreeCamera::NodeConstructorAdded = true;
}

StereoscopicFreeCamera::StereoscopicFreeCamera(const std::string& iName, const Vector3& iPosition,
