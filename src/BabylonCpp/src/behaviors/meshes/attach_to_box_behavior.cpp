...
AttachToBoxBehavior::AttachToBoxBehavior(const TransformNodePtr& iUi)
    ...
    , ui{iUi}
    , _faceVectors{{FaceDirectionInfo{Vector3::Up()}, FaceDirectionInfo{Vector3::Down()},
                    FaceDirectionInfo{Vector3::Left()}, FaceDirectionInfo{Vector3::Right()},
...
FaceDirectionInfo AttachToBoxBehavior::_closestFace(const Vector3& targetDirection)
{
  ...
  for (auto& v : _faceVectors) {
    ...
    Vector3::TransformCoordinatesToRef(v.direction, _tmpMatrix, v.rotatedDirection);
    v.diff = Vector3::GetAngleBetweenVectors(v.rotatedDirection, targetDirection,
  ...
  const auto determineMin = [](const FaceDirectionInfo& min, const FaceDirectionInfo& p) {
    if (min.ignore) {
      return p;
    }
    else if (p.ignore) {
...
void AttachToBoxBehavior::_lookAtToRef(const Vector3& pos, Quaternion& ref, const Vector3 up)
{
                                 ...
                                 const std::function<bool(const AbstractMeshPtr& m)>& /*predicate*/)
{
  ...
  _onRenderObserver = _scene->onBeforeRenderObservable.add([this, target](Scene* /*scene*/,
                                                                          EventState& /*es*/) {
    ...
    if (_scene->activeCamera()->leftCamera()) {
      _scene->activeCamera()->leftCamera()->computeWorldMatrix().getRotationMatrixToRef(_tmpMatrix);
    }
    else {
      _scene->activeCamera()->computeWorldMatrix().getRotationMatrixToRef(_tmpMatrix);
    }
    ...
    if (facing.direction.x != 0.f) {
      facing.rotatedDirection.scaleToRef((target->scaling().x / 2.f) + distanceAwayFromFace,
                                         _tmpVector);
      ui->position().addInPlace(_tmpVector);
    }
    if (facing.direction.y != 0.f) {
      facing.rotatedDirection.scaleToRef((target->scaling().y / 2.f) + distanceAwayFromFace,
                                         _tmpVector);
      ui->position().addInPlace(_tmpVector);
    }
    if (facing.direction.z != 0.f) {
      facing.rotatedDirection.scaleToRef((target->scaling().z / 2.f) + distanceAwayFromFace,
                                         _tmpVector);
      ui->position().addInPlace(_tmpVector);
    }
    ...
    if (!ui->rotationQuaternion()) {
      ui->rotationQuaternion
        = Quaternion::RotationYawPitchRoll(ui->rotation().y, ui->rotation().x, ui->rotation().z);
    }
    facing.rotatedDirection.scaleToRef(-1, _tmpVector);
    _lookAtToRef(_tmpVector, *ui->rotationQuaternion(), facingUp.rotatedDirection);

    // Place ui the correct distance from the bottom of the mesh
    if (facingUp.direction.x != 0.f) {
      ui->up().scaleToRef(distanceAwayFromBottomOfFace - target->scaling().x / 2.f, _tmpVector);
    }
    if (facingUp.direction.y != 0.f) {
      ui->up().scaleToRef(distanceAwayFromBottomOfFace - target->scaling().y / 2.f, _tmpVector);
    }
    if (facingUp.direction.z != 0.f) {
      ui->up().scaleToRef(distanceAwayFromBottomOfFace - target->scaling().z / 2.f, _tmpVector);
    }
    ui->position().addInPlace(_tmpVector);
  });
}
