...
Curve3 Curve3::continueCurve3(Curve3& curve) const
{
  ...
  for (unsigned int i = 1; i < curvePoints.size(); ++i) {
    continuedPoints.emplace_back(curvePoints[i].subtract(curvePoints[0]).add(lastPoint));
  }
  return Curve3(continuedPoints);
}

Curve3 Curve3::CreateQuadraticBezier(const Vector3& v0, const Vector3& v1, const Vector3& v2,
                                     int nbPoints)
{
...
Curve3 Curve3::CreateCubicBezier(const Vector3& v0, const Vector3& v1, const Vector3& v2,
                                 const Vector3& v3, int nbPoints)
{
  const float nbPointsVal = nbPoints > 3 ? static_cast<float>(nbPoints) : 4.f;
  std::vector<Vector3> bez;
  const auto equation = [](float t, float val0, float val1, float val2, float val3) -> float {
...
Curve3 Curve3::CreateHermiteSpline(const Vector3& p1, const Vector3& t1, const Vector3& p2,
...
Curve3 Curve3::CreateCatmullRomSpline(const std::vector<Vector3>& points, size_t nbPoints,
                                      bool closed)
{
  ...
  if (closed) {
    ...
    for (size_t i = 0; i < pointsCount; ++i) {
      ...
      for (size_t c = 0; c < nbPoints; ++c) {
        catmullRom.emplace_back(Vector3::CatmullRom(
          points[i % pointsCount], points[(i + 1) % pointsCount], points[(i + 2) % pointsCount],
    ...
    for (i = 0; i < totalPoints.size() - 3; ++i) {
      ...
      for (size_t c = 0; c < nbPoints; ++c) {
        catmullRom.emplace_back(Vector3::CatmullRom(
          totalPoints[i], totalPoints[i + 1], totalPoints[i + 2], totalPoints[i + 3], amount));
        amount += step;
      }
    }
    --i;
    catmullRom.emplace_back(Vector3::CatmullRom(totalPoints[i], totalPoints[i + 1],
