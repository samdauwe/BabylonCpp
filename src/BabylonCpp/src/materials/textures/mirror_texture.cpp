...
void MirrorTexture::_autoComputeBlurKernel()
{
  ...
  const auto dw
    = static_cast<float>(getRenderWidth()) / static_cast<float>(engine->getRenderWidth());
  const auto dh
    = static_cast<float>(getRenderHeight()) / static_cast<float>(engine->getRenderHeight());
