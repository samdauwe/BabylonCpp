...
RawCubeTexture::RawCubeTexture(Scene* scene, const std::vector<ArrayBufferView>& data, int size,
                               unsigned int format, unsigned int type, bool generateMipMaps,
                               bool invertY, unsigned int samplingMode,
                               const std::string& compression)
    : CubeTexture{"", scene}
{
  _texture = scene->getEngine()->createRawCubeTexture(data, size, format, type, generateMipMaps,
                                                      invertY, samplingMode, compression);
}

RawCubeTexture::~RawCubeTexture() = default;

void RawCubeTexture::update(const std::vector<ArrayBufferView>& data, unsigned int format,
