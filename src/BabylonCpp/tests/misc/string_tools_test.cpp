...
TEST(TestString, toCharCodes)
{
  ...
  EXPECT_THAT(StringTools::toCharCodes(s), ::testing::ContainerEq(expected));
}
...
TEST(TestString, concat)
{
  ...
  EXPECT_EQ(StringTools::concat(1, "2", "A"), "12A");
}
...
TEST(TestString, contains)
{
  ...
  const std::string s("Babylon.js");
  EXPECT_TRUE(StringTools::contains(s, "Babylon"));
  EXPECT_FALSE(StringTools::contains(s, "babylon"));
}
...
TEST(TestString, count)
{
  ...
  const std::string s("-- Babylon.js --");
  EXPECT_EQ(StringTools::count(s, "B"), 1);
  EXPECT_EQ(StringTools::count(s, "."), 1);
  EXPECT_EQ(StringTools::count(s, "z"), 0);
  EXPECT_EQ(StringTools::count(s, "-"), 4);
  EXPECT_EQ(StringTools::count(s, "--"), 2);
}
...
TEST(TestString, escape)
{
  ...
  EXPECT_THAT(StringTools::escape("*"), "\\*");
  EXPECT_THAT(StringTools::escape("\\"), "\\\\");
  EXPECT_THAT(StringTools::escape("||"), "\\|\\|");
  EXPECT_THAT(StringTools::escape(","), ",");
}
...
TEST(TestString, escapeStrings)
{
  ...
  EXPECT_THAT(StringTools::escape(input), ::testing::ContainerEq(expected));
}
...
TEST(TestString, startsWith)
{
  ...
  const std::string s("Babylon.js");
  EXPECT_TRUE(StringTools::startsWith(s, "Babylon"));
  EXPECT_FALSE(StringTools::startsWith(s, "babylon"));
  EXPECT_FALSE(StringTools::startsWith(s, "Babylon.js_"));
}
...
TEST(TestString, endsWith)
{
  ...
  const std::string s("Babylon.js");
  EXPECT_TRUE(StringTools::endsWith(s, ".js"));
  EXPECT_FALSE(StringTools::endsWith(s, ".JS"));
  EXPECT_FALSE(StringTools::endsWith(s, "Babylon.js_"));
}
...
TEST(TestString, fromCharCode)
{
  ...
  EXPECT_EQ(StringTools::fromCharCode(65), "A");
  EXPECT_EQ(StringTools::fromCharCode(72, 69, 76, 76, 79), "HELLO");
}
...
TEST(TestString, indexOf)
{
  ...
  EXPECT_EQ(StringTools::indexOf(str, "welcome"), 13);
  EXPECT_EQ(StringTools::indexOf(str, "e"), 1);
  EXPECT_EQ(StringTools::indexOf(str, "e", 5), 14);
  EXPECT_EQ(StringTools::indexOf(str, "abc"), -1);
}
...
TEST(TestString, isDigit)
{
  ...
  EXPECT_TRUE(StringTools::isDigit(5));
  EXPECT_TRUE(StringTools::isDigit("5"));
  EXPECT_FALSE(StringTools::isDigit("A"));
  EXPECT_FALSE(StringTools::isDigit("*"));
}
...
TEST(TestString, join)
{
  ...
  EXPECT_EQ(StringTools::join(v, ','), expected);
}
...
TEST(TestString, lastIndexOf)
{
  ...
  EXPECT_EQ(StringTools::lastIndexOf(str, "planet"), 36);
  EXPECT_EQ(StringTools::lastIndexOf(str, "planet", 20), 6);
  EXPECT_EQ(StringTools::lastIndexOf(str, "abc"), -1);
}
...
TEST(TestString, nthChar)
{
  ...
  EXPECT_EQ(StringTools::nthChar("abc", 0), 'a');
  EXPECT_EQ(StringTools::nthChar("abc", 1), 'b');
  EXPECT_EQ(StringTools::nthChar("abc", 2), 'c');
}
...
TEST(TestString, pad)
{
  ...
  StringTools::pad(s, 15, '.');
  EXPECT_EQ(s, "Babylon.js.....");
  StringTools::pad(s, 10, '.');
  EXPECT_EQ(s, "Babylon.js.....");
}
...
TEST(TestString, printf)
{
  ...
  EXPECT_EQ(StringTools::printf("%s", "Hello"), "Hello");
  EXPECT_EQ(StringTools::printf("Pi = %.5lf", 3.14159), "Pi = 3.14159");

  auto test
    = StringTools::printf("Hello %s, did you know that pi equals %.2lf?\n", "User", 3.14159);
...
TEST(TestString, randomString)
{
  ...
  const std::string r1 = StringTools::randomString(10);
  EXPECT_EQ(r1.size(), 10);

  const std::string r2 = StringTools::randomString(5, "a");
  EXPECT_EQ(r2, "aaaaa");
}
...
TEST(TestString, regexMatch)
{
...

  {
    ...
    const auto r = StringTools::regexMatch(s, re);
...
TEST(TestString, regexReplace)
{
...

  {
    ...
    const std::string r{StringTools::regexReplace(s, regex, "in ")};
...

  {
    ...
    const std::string r{StringTools::regexReplace(s, regex, "")};
    const std::string e;
    EXPECT_EQ(r, e);
  }
...

  {
    ...
    const std::string r{StringTools::regexReplace(s, "varying\\s", "in ")};
...

  {
    ...
    const std::string r{StringTools::regexReplace(s, "attribute[ \\t]", "in ")};
...

  {
    ...
    const std::string r{StringTools::regexReplace(s, "texture2D\\(", "texture(")};
...

  {
    ...
    const std::string r{
      StringTools::regexReplace(s, "void\\s+?main\\(", "out vec4 glFragColor;\nvoid main(")};
...

  {
    const auto _glslFloat = [](float x, unsigned int decimalFigures) {
      ...
      oss << std::fixed << x;
      return StringTools::regexReplace(oss.str(), "0+", "");
    };
...
TEST(TestString, regexReplaceWithCallback)
{
  ...
  // Using free function
  {
    ...
    const std::string r{
      StringTools::regexReplace(s, std::regex("\\d+", std::regex::optimize), my_callback)};
  ...
  // Using lambda function
  {
    ...
    const std::string s{"1, 9, 19"};
    const std::string r{
      StringTools::regexReplace(s, std::regex(R"(\d+)", std::regex::optimize), callback)};
    const std::string e{"2, 10, 20"};
    EXPECT_EQ(r, e);
  }

  {
    ...
    const std::string r{StringTools::regexReplace(s, regex, callback)};
    const std::string e{
  ...
  // Note: only integer based
  {
    const auto f2c = [](const std::string& x) {
      ...
      return StringTools::regexReplace(x, std::regex("(\\d+)F", std::regex::optimize), convert);
    };
  ...
  // Example: number lines
  {
    ...
    const std::string r{
      "1\t" + StringTools::regexReplace(s, std::regex("\n", std::regex::optimize), callback)};
...
TEST(TestString, removeSubstring)
{
  ...
  EXPECT_EQ(StringTools::removeSubstring(s, subStr), expected);
}
...
TEST(TestString, repeat)
{
  ...
  EXPECT_EQ(StringTools::repeat("Hello world!", 0), "");
  EXPECT_EQ(StringTools::repeat("Hello world!", 2), "Hello world!Hello world!");
}
...
TEST(TestString, replace)
{
  ...
  StringTools::replaceInPlace(source, " ", "");
...
TEST(TestString, split)
{
  ...
  EXPECT_THAT(StringTools::split(str, ','), ::testing::ContainerEq(expected));
}
...
TEST(TestString, slice)
{
  ...
  EXPECT_EQ(StringTools::slice(str), "Hello world!");
  EXPECT_EQ(StringTools::slice(str, 0), "Hello world!");
  EXPECT_EQ(StringTools::slice(str, 3), "lo world!");
}
...
TEST(TestString, stripComments)
{
...

  {
    ...
    EXPECT_EQ(StringTools::stripComments(str, "//"), "");
  }
...

  {
    ...
    EXPECT_EQ(StringTools::stripComments(str, "//"), "Hello world!");
  }

  {
    const std::string str{"const float a = 1.0 / b;"};
    EXPECT_EQ(StringTools::stripComments(str, "//"), str);
  }

  {
    const std::string str{"a /= b; // A Comment"};
    EXPECT_EQ(StringTools::stripComments(str, "//"), "a /= b;");
  }
}
...
TEST(TestString, toLowerCase)
{
  ...
  EXPECT_EQ(StringTools::toLowerCase(source), expected);
}
...
TEST(TestString, toUpperCase)
{
  ...
  EXPECT_EQ(StringTools::toUpperCase(source), expected);
}
...
TEST(TestString, toNumber)
{
  ...
  EXPECT_EQ(StringTools::toNumber<int>("3"), 3);
  EXPECT_EQ(StringTools::toNumber<size_t>("3"), 3);
  EXPECT_EQ(StringTools::toNumber<float>("4.5"), 4.5f);
  EXPECT_EQ(StringTools::toNumber<double>("4.5"), 4.5);
}
...
TEST(TestString, toString)
{
  ...
  EXPECT_EQ(StringTools::toString<int>(3), "3");
  EXPECT_EQ(StringTools::toString<size_t>(3), "3");
  EXPECT_EQ(StringTools::toString<float>(4.5f), "4.5");
  EXPECT_EQ(StringTools::toString<double>(4.5), "4.5");
}
...
TEST(TestString, toTitleCase)
{
  ...
  EXPECT_EQ(StringTools::toTitleCase(str), "Hello World");
}
...
TEST(TestString, trim)
{
  ...
  std::string str{"  abc  "};
  EXPECT_EQ(StringTools::trim(str), "abc");
}
...
TEST(TestString, trimCopy)
{
  ...
  std::string str{"  abc "};
  std::string strCopy{StringTools::trimCopy(str)};
  EXPECT_EQ(str, "  abc ");
...
TEST(TestString, trimLeft)
{
  ...
  std::string str{"  abc "};
  EXPECT_EQ(StringTools::trimLeft(str), "abc ");
}
...
TEST(TestString, trimRight)
{
  ...
  std::string str{" abc  "};
  EXPECT_EQ(StringTools::trimRight(str), " abc");
}
