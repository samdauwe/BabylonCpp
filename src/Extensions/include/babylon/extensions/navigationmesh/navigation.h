...
public:
  ...
  std::vector<Vector3> findPath(const Vector3& startPosition, const Vector3& targetPosition,
                                const std::string& zone, std::size_t group);

private:
  [[nodiscard]] bool _isPointInPoly(const std::vector<Vector3>& poly, const Vector3& pt) const;
  ...
  void _setPolygonCentroid(NavigationPolygon& polygon, const NavigationMesh& navigationMesh);
  Vector3 getVectorFrom(const Float32Array& vertices, unsigned int id);
  std::vector<std::vector<NavigationPolygon>> _buildPolygonGroups(NavigationMesh& navigationMesh);
  Uint32Array _array_intersect(const Uint32Array& array1, const Uint32Array& array2);
  void _buildPolygonNeighbours(NavigationPolygon& polygon, NavigationMesh& navigationMesh);
  ...
  Uint32Array _getSharedVerticesInOrder(NavigationPolygon& a, NavigationPolygon& b);
