...
public:
   ...
   * A larger array of random unit length vectors would also do the job, but these 12 (including 4
   * repeats to make the array length a power of two) work better. They are not random, they are
   * carefully chosen to represent a small, isotropic set of directions.
   */
   ...
   * These vectors are based on the midpoints of the 12 edges of a cube, they all rotate in their
   * own plane and are never coincident or collinear. A larger array of random vectors would also do
   * the job, but these 12 (including 4 repeats to make the array length a power of two) work
   * better. They are not random, they are carefully chosen to represent a small isotropic set of
   * directions for any rotation angle.
   */
...
private:
   ...
   * Note that these generate gradients of more than unit length. To make a close match with the
   * value range of classic Perlin noise, the final noise values need to be rescaled to fit nicely
   * within [-1,1]. (The simplex noise functions as such also have different scaling.) Note also
   * that these noise functions are the most practical and useful signed version of Perlin noise. To
   * return values according to the RenderMan specification from the SL noise() and pnoise()
   * functions, the noise values need to be scaled and offset to [0,1], like this: float SLnoise =
   * (SimplexNoise1234::noise(x,y,z) + 1.0) * 0.5;
   */
  ...
  /*
   * Helper functions to compute gradients in 1D to 4D and gradients-dot-residualvectors in 2D to
   * 4D.
   */
...
private:
  /*
   * Permutation table. This is just a random jumble of all numbers 0-255, repeated twice to avoid
   * wrapping the index at 255 for each lookup. This needs to be exactly the same for all instances
   * on all platforms, so it's easiest to just keep it as static explicit data. This also removes
   * the need for any initialisation of this class.
   *
   * Note that making this an int[] instead of a char[] might make the code run faster on platforms
   * with a high penalty for unaligned single byte addressing. Intel x86 is generally
   * single-byte-friendly, but some other CPUs are faster with 4-aligned reads. However, a char[] is
   * smaller, which avoids cache trashing, and that is probably the most important aspect on most
   * architectures. This array is accessed a *lot* by the noise functions. A vector-valued noise
   * over 3D accesses it 96 times, and a float-valued 4D noise 64 times. We want this to fit in the
   * cache!
   */
