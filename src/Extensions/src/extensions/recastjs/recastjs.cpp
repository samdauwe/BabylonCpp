...
int NavMesh::rasterizeTileLayers(const int tx, const int ty, const rcConfig& cfg,
                                 ...
                                 const std::vector<float>& verts)
{
  ...
  if (!rcCreateHeightfield(&ctx, *tileIntermediates.m_solid, tcfg.width, tcfg.height, tcfg.bmin,
  ...
  if (!rcBuildCompactHeightfield(&ctx, tcfg.walkableHeight, tcfg.walkableClimb,
                                 *tileIntermediates.m_solid, *tileIntermediates.m_chf)) {
  ...
  if (!rcBuildHeightfieldLayers(&ctx, *tileIntermediates.m_chf, tcfg.borderSize,
                                tcfg.walkableHeight, *tileIntermediates.m_lset)) {
  ...
  for (int i = 0; i < rcMin(tileIntermediates.m_lset->nlayers, MAX_LAYERS); ++i) {
    ...
    dtStatus status = dtBuildTileCacheLayer(&comp, &header, layer->heights, layer->areas,
...
bool NavMesh::computeTiledNavMesh(const std::vector<float>& verts, const std::vector<int>& tris,
                                  rcConfig& cfg, NavMeshintermediates& intermediates,
                                  const std::vector<unsigned char>& triareas)
{
  dtStatus status;
  ...
  for (int y = 0; y < th; ++y) {
    for (int x = 0; x < tw; ++x) {
      ...
      int ntiles
        = rasterizeTileLayers(x, y, cfg, tiles, MAX_LAYERS, intermediates, triareas, verts);
                    ...
                    const int indexCount, const rcConfig& config)
{
  ...
  if (config.tileSize) {
    ...
    if (!rcCreateHeightfield(&ctx, *intermediates.m_solid, cfg.width, cfg.height, cfg.bmin,
                             cfg.bmax, cfg.cs, cfg.ch)) {
      Log("buildNavigation: Could not create solid heightfield.");
      return;
    }

    rcRasterizeTriangles(&ctx, verts.data(), nverts, tris.data(), triareas.data(), ntris,
