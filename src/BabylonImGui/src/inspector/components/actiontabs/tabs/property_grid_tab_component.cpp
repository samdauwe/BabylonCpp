...
#include <babylon/inspector/components/actiontabs/tabs/propertygrids/cameras/follow_camera_property_grid_component.h>
...
PropertyGridTabComponent::PropertyGridTabComponent(const IPaneComponentProps& iProps)
...
void PropertyGridTabComponent::componentWillMount()
{
  ...
  _scenePropertyGridComponent = std::make_unique<ScenePropertyGridComponent>(sceneProps);
}
...
void PropertyGridTabComponent::render()
{
  ...
  if (entity->type == EntityType::Mesh) {
    ...
    if (!mesh || mesh->uniqueId != entityId) {
      mesh = std::dynamic_pointer_cast<Mesh>(props.scene->getMeshByUniqueID(entityId));
      _reservedDataStore.mesh[mesh->uniqueId] = MeshReservedDataStore{};
    }
    if (mesh) {
      if (mesh->getTotalVertices() > 0) {
        MeshPropertyGridComponent::render(mesh, _reservedDataStore.mesh[mesh->uniqueId]);
      }
      else {
  ...
  if (entity->type == EntityType::FreeCamera || entity->type == EntityType::UniversalCamera) {
    auto& freeCamera = _entityCache.freeCamera;
    if (!freeCamera || freeCamera->uniqueId != entityId) {
      freeCamera
        = std::dynamic_pointer_cast<FreeCamera>(props.scene->getCameraByUniqueID(entityId));
    }
    if (freeCamera) {
  ...
  if (entity->type == EntityType::ArcRotateCamera) {
    ...
    if (!arcRotateCamera || arcRotateCamera->uniqueId != entityId) {
      arcRotateCamera
        = std::dynamic_pointer_cast<ArcRotateCamera>(props.scene->getCameraByUniqueID(entityId));
    }
    if (arcRotateCamera) {
  ...
  if (entity->type == EntityType::FollowCamera) {
    auto& followCamera = _entityCache.followCamera;
    if (!followCamera || followCamera->uniqueId != entityId) {
      followCamera
        = std::dynamic_pointer_cast<FollowCamera>(props.scene->getCameraByUniqueID(entityId));
    }
    if (followCamera) {
      FollowCameraPropertyGridComponent::render(followCamera);
    }
    return;
  }
  ...
  if (entity->type == EntityType::HemisphericLight) {
    ...
    if (!hemisphericLight || hemisphericLight->uniqueId != entityId) {
      hemisphericLight
        = std::dynamic_pointer_cast<HemisphericLight>(props.scene->getLightByUniqueID(entityId));
    }
    if (hemisphericLight) {
  ...
  if (entity->type == EntityType::PointLight) {
    ...
    if (!pointLight || pointLight->uniqueId != entityId) {
      pointLight = std::dynamic_pointer_cast<PointLight>(props.scene->getLightByUniqueID(entityId));
    }
    if (pointLight) {
  ...
  if (entity->type == EntityType::DirectionalLight) {
    ...
    if (!directionalLight || directionalLight->uniqueId != entityId) {
      directionalLight
        = std::dynamic_pointer_cast<DirectionalLight>(props.scene->getLightByUniqueID(entityId));
    }
    if (directionalLight) {
  ...
  if (entity->type == EntityType::SpotLight) {
    ...
    if (!spotLight || spotLight->uniqueId != entityId) {
      spotLight = std::dynamic_pointer_cast<SpotLight>(props.scene->getLightByUniqueID(entityId));
    }
    if (spotLight) {
  ...
  if (entity->type == EntityType::MultiMaterial) {
    ...
    if (!multiMaterial || multiMaterial->uniqueId != entityId) {
      multiMaterial
        = std::dynamic_pointer_cast<MultiMaterial>(props.scene->getMaterialByUniqueID(entityId));
    }
    if (multiMaterial) {
  ...
  if (entity->type == EntityType::StandardMaterial) {
    ...
    if (!standardMaterial || standardMaterial->uniqueId != entityId) {
      standardMaterial
        = std::dynamic_pointer_cast<StandardMaterial>(props.scene->getMaterialByUniqueID(entityId));
    }
    if (standardMaterial) {
  ...
  if (entity->type == EntityType::PBRMaterial) {
    ...
    if (!pbrMaterial || pbrMaterial->uniqueId != entityId) {
      pbrMaterial
        = std::dynamic_pointer_cast<PBRMaterial>(props.scene->getMaterialByUniqueID(entityId));
    }
    if (pbrMaterial) {
  ...
  if (entity->type == EntityType::PBRMetallicRoughnessMaterial) {
    auto& pbrMetallicRoughnessMaterial = _entityCache.pbrMetallicRoughnessMaterial;
    if (!pbrMetallicRoughnessMaterial || pbrMetallicRoughnessMaterial->uniqueId != entityId) {
      pbrMetallicRoughnessMaterial = std::dynamic_pointer_cast<PBRMetallicRoughnessMaterial>(
        props.scene->getMaterialByUniqueID(entityId));
    }
    if (pbrMetallicRoughnessMaterial) {
      PBRMetallicRoughnessMaterialPropertyGridComponent::render(pbrMetallicRoughnessMaterial);
    }
    return;
  }

  if (entity->type == EntityType::PBRSpecularGlossinessMaterial) {
    auto& pbrSpecularGlossinessMaterial = _entityCache.pbrSpecularGlossinessMaterial;
    if (!pbrSpecularGlossinessMaterial || pbrSpecularGlossinessMaterial->uniqueId != entityId) {
      pbrSpecularGlossinessMaterial = std::dynamic_pointer_cast<PBRSpecularGlossinessMaterial>(
        props.scene->getMaterialByUniqueID(entityId));
    }
    if (pbrSpecularGlossinessMaterial) {
      PBRSpecularGlossinessMaterialPropertyGridComponent::render(pbrSpecularGlossinessMaterial);
    }
    return;
  }
  ...
  if (entity->type == EntityType::Material) {
    ...
    if (!material || material->uniqueId != entityId) {
      material = std::dynamic_pointer_cast<Material>(props.scene->getMaterialByUniqueID(entityId));
    }
    if (material) {
  ...
  if (entity->type == EntityType::Texture) {
    ...
    if (!texture || texture->name != entityName) {
      auto it = std::find_if(
        props.scene->textures.begin(), props.scene->textures.end(),
        [&entityName](const BaseTexturePtr& texture) { return texture->name == entityName; });
      texture
        = (it == props.scene->textures.end()) ? nullptr : std::static_pointer_cast<Texture>(*it);
      _reservedDataStore.texture[texture->name] = TextureReservedDataStore{};
    }
    if (texture) {
      TexturePropertyGridComponent::render(texture, _reservedDataStore.texture[texture->name]);
    }
    return;
  }
  ...
  if (entity->type == EntityType::Skeleton) {
    ...
    if (!skeleton || skeleton->uniqueId != entityId) {
      skeleton = std::dynamic_pointer_cast<Skeleton>(props.scene->getSkeletonByUniqueID(entityId));
      _reservedDataStore.skeleton[skeleton->uniqueId]        = SkeletonReservedDataStore{};
      _reservedDataStore.animation[skeleton->uniqueId]       = AnimationReservedDataStore{};
      _reservedDataStore.animation[skeleton->uniqueId].scene = props.scene;
    }
    if (skeleton) {
      SkeletonPropertyGridComponent::render(skeleton,
  ...
  if (entity->type == EntityType::Bone) {
    ...
    if (!bone || bone->uniqueId != entityId) {
      bone = std::dynamic_pointer_cast<Bone>(props.scene->getBoneByUniqueID(entityId));
    }
    if (bone) {
