...
struct BABYLON_SHARED_EXPORT TexturePropertyGridComponent {
                     ...
                     TextureReservedDataStore& textureReservedDataStore)
  {
    ...
    static std::vector<std::pair<const char*, unsigned int>> coordinatesMode{
      ...
      {"Fixed equirectangular mirrored", TextureConstants::FIXED_EQUIRECTANGULAR_MIRRORED_MODE},
    ...
    if (ImGui::CollapsingHeader("GENERAL")) {
      TextLineComponent::render("Width", std::to_string(texture->getSize().width));
      TextLineComponent::render("Height", std::to_string(texture->getSize().height));
      TextLineComponent::render("Unique ID", std::to_string(texture->uniqueId));
      TextLineComponent::render("Class", texture->getClassName());
      TextLineComponent::render("Has alpha", texture->hasAlpha() ? "Yes" : "No");
      TextLineComponent::render("Is 3D", texture->is3D ? "Yes" : "No");
      TextLineComponent::render("Is cube", texture->isCube ? "Yes" : "No");
      TextLineComponent::render("Is render target", texture->isRenderTarget ? "Yes" : "No");
      auto sliderChange = SliderLineComponent::render(
        "UV set", static_cast<float>(texture->coordinatesIndex), 0.f, 3.f, 1.f, "%.3f");
      if (sliderChange) {
        texture->coordinatesIndex = static_cast<unsigned int>(sliderChange.value());
      }
      if (_texture) {
        auto optionChange
          = OptionsLineComponent::render("Mode", _texture->coordinatesMode(), coordinatesMode);
        if (optionChange) {
          _texture->updateSamplingMode(optionChange.value());
        }
      }
      sliderChange = SliderLineComponent::render("Level", texture->level, 0.f, 2.f, 0.01f, "%.2f");
      if (sliderChange) {
        texture->level = sliderChange.value();
      }
      if (_texture) {
        auto optionChange
          = OptionsLineComponent::render("Sampling", _texture->samplingMode(), samplingMode);
        if (optionChange) {
          _texture->updateSamplingMode(optionChange.value());
        }
      }
    ...
    if (ImGui::CollapsingHeader("TRANSFORM")) {
      if (!texture->isCube && _texture) {
        auto valueChange = FloatLineComponent::render("U offset", _texture->uOffset);
        if (valueChange) {
        ...
        if (CheckBoxLineComponent::render("Clamp U",
        ...
        if (CheckBoxLineComponent::render("Clamp V",
        ...
        if (cubeTexture) {
          auto sliderChange = SliderLineComponent::render("Rotation Y", cubeTexture->rotationY(),
                                                          0.f, Math::PI2, 0.1f, "%.1f");
          if (sliderChange) {
            cubeTexture->rotationY = static_cast<float>(sliderChange.value());
          }
        }
        else {
          auto hdrCubeTexture = std::static_pointer_cast<HDRCubeTexture>(texture);
          if (hdrCubeTexture) {
            auto sliderChange = SliderLineComponent::render(
              "Rotation Y", hdrCubeTexture->rotationY(), 0, Math::PI2, 0.1f, "%.1f");
            if (sliderChange) {
              hdrCubeTexture->rotationY = static_cast<float>(sliderChange.value());
            }
          }
        }
      }
