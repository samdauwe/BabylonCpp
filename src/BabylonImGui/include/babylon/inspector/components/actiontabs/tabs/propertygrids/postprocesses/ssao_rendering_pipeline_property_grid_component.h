...
struct BABYLON_SHARED_EXPORT SSAORenderingPipelinePropertyGridComponent {
  ...
  static void render(const SSAORenderingPipelinePtr& renderPipeline)
  {
    ...
    if (ImGui::CollapsingHeader("SSAO")) {
      ...
      sliderChange = SliderLineComponent::render("Area", renderPipeline->area, 0.0001f, 0.01f,
                                                 0.0001f, "%.4f");
      if (sliderChange) {
        renderPipeline->area = sliderChange.value();
      }
      sliderChange = SliderLineComponent::render("Falloff", renderPipeline->fallOff, 0.f, 0.00001f,
                                                 0.000001f, "%.6f");
      if (sliderChange) {
