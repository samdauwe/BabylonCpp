...
struct BABYLON_SHARED_EXPORT DefaultRenderingPipelinePropertyGridComponent {
  ...
  static void render(const DefaultRenderingPipelinePtr& renderPipeline)
  {
    ...
    if (ImGui::CollapsingHeader("BLOOM")) {
      ...
      if (renderPipeline->bloomEnabled()) {
        auto sliderChange = SliderLineComponent::render(
          "Threshold", renderPipeline->bloomThreshold(), 0.f, 1.f, 0.05f,
          "%.2f");
        if (sliderChange) {
          renderPipeline->bloomThreshold = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Weight", renderPipeline->bloomWeight(), 0.f, 1.f, 0.05f, "%.2f");
        if (sliderChange) {
          renderPipeline->bloomWeight = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Kernel", renderPipeline->bloomKernel(), 0.f, 128.f, 1.f, "%.0f");
        if (sliderChange) {
          renderPipeline->bloomKernel = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Scale", renderPipeline->bloomScale(), 0.f, 1.f, 0.25f, "%.2f");
        if (sliderChange) {
    ...
    ImGui::SetNextTreeNodeOpen(chromaticAberrationContainerOpened,
                               ImGuiCond_Always);
    if (ImGui::CollapsingHeader("CHROMATIC ABERRATION")) {
      if (CheckBoxLineComponent::render(
            "Enabled", renderPipeline->chromaticAberrationEnabled())) {
      ...
      if (renderPipeline->chromaticAberrationEnabled()) {
        auto sliderChange = SliderLineComponent::render(
          "aberrationAmount",
          renderPipeline->chromaticAberration->aberrationAmount, 0.f, 128.f,
          0.1f, "%.1f");
        if (sliderChange) {
          renderPipeline->chromaticAberration->aberrationAmount
            = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Radial intensity",
          renderPipeline->chromaticAberration->radialIntensity, 0.f, 1.f, 0.01f,
          "%.2f");
        if (sliderChange) {
          renderPipeline->chromaticAberration->radialIntensity
            = sliderChange.value();
        }
        Vector2LineComponent::render(
          "Center", renderPipeline->chromaticAberration->centerPosition);
        Vector2LineComponent::render(
          "Direction", renderPipeline->chromaticAberration->direction);
      }
    ...
    if (ImGui::CollapsingHeader("DEPTH OF FIELD")) {
      if (CheckBoxLineComponent::render(
            "Enabled", renderPipeline->depthOfFieldEnabled())) {
        renderPipeline->depthOfFieldEnabled
          = !renderPipeline->depthOfFieldEnabled();
      }
      if (renderPipeline->depthOfFieldEnabled()) {
        auto sliderChange = SliderLineComponent::render(
          "Focal length", renderPipeline->depthOfField->focalLength(), 0.f,
          camera->maxZ, 1.f, "%.1f");
        if (sliderChange) {
          renderPipeline->depthOfField->focalLength = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "fStop", renderPipeline->depthOfField->fStop(), 0.f, 32.f, 0.1f,
          "%.1f");
        if (sliderChange) {
          renderPipeline->depthOfField->fStop = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Distance", renderPipeline->depthOfField->focusDistance(), 0.f,
          camera->maxZ, 0.1f, "%.1f");
        if (sliderChange) {
          renderPipeline->depthOfField->focusDistance = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Lens size", renderPipeline->depthOfField->lensSize(), 0.f, 1000.f,
          1.f, "%.0f");
        if (sliderChange) {
    ...
    if (ImGui::CollapsingHeader("GLOW LAYER")) {
      ...
      if (renderPipeline->glowLayerEnabled()) {
        auto sliderChange = SliderLineComponent::render(
          "Blur kernel size", renderPipeline->glowLayer()->blurKernelSize(),
          1.f, 128.f, 1.f, "%.0f");
        if (sliderChange) {
          renderPipeline->glowLayer()->blurKernelSize = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Intensity", renderPipeline->glowLayer()->intensity(), 0.f, 10.f,
          0.1f, "%.1f");
        if (sliderChange) {
    ...
    if (ImGui::CollapsingHeader("GRAIN")) {
      ...
      if (renderPipeline->grainEnabled()) {
        if (CheckBoxLineComponent::render("Animated",
                                          renderPipeline->grain->animated)) {
          renderPipeline->grain->animated = !renderPipeline->grain->animated;
        }
        auto sliderChange = SliderLineComponent::render(
          "Intensity", renderPipeline->grain->intensity, 0.f, 50.f, 0.1f,
          "%.1f");
        if (sliderChange) {
    ...
    ImGui::SetNextTreeNodeOpen(imageProcessingContainerOpened,
                               ImGuiCond_Always);
    if (ImGui::CollapsingHeader("IMAGE PROCESSING")) {
      if (CheckBoxLineComponent::render(
            "Enabled", renderPipeline->imageProcessingEnabled())) {
        renderPipeline->imageProcessingEnabled
          = !renderPipeline->imageProcessingEnabled();
      }
      const auto imageProcessing
        = std::static_pointer_cast<ImageProcessingPostProcess>(
      ...
      if (imageProcessing) {
        ...
        sliderChange = SliderLineComponent::render(
          "Vignette weight", imageProcessing->vignetteWeight, 0.f, 4.f, 0.1f,
          "%.2f");
        if (sliderChange) {
          imageProcessing->vignetteWeight = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Vignette stretch", imageProcessing->vignetteStretch, 0.f, 1.f, 0.1f,
          "%.2f");
        if (sliderChange) {
          imageProcessing->vignetteStretch = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Vignette FOV", imageProcessing->vignetteCameraFov, 0.f, Math::PI,
          0.1f, "%.2f");
        if (sliderChange) {
          imageProcessing->vignetteCameraFov = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Vignette center X", imageProcessing->vignetteCentreX, 0.f, 1.f, 0.1f,
          "%.2f");
        if (sliderChange) {
          imageProcessing->vignetteCentreX = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Vignette center Y", imageProcessing->vignetteCentreY, 0.f, 1.f, 0.1f,
          "%.2f");
        if (sliderChange) {
    ...
    if (ImGui::CollapsingHeader("SHARPEN")) {
      ...
      if (renderPipeline->sharpenEnabled()) {
        auto sliderChange = SliderLineComponent::render(
          "Color amount", renderPipeline->sharpen->colorAmount, 0.f, 1.f, 0.05f,
          "%.2f");
        if (sliderChange) {
          renderPipeline->sharpen->colorAmount = sliderChange.value();
        }
        sliderChange = SliderLineComponent::render(
          "Edge amount", renderPipeline->sharpen->edgeAmount, 0.f, 5.f, 0.05f,
          "%.2f");
        if (sliderChange) {
