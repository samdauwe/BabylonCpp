...
struct DockContext {
    ...
    void setChildrenPosSize(const ImVec2& _pos, const ImVec2& _size)
    {
      ...
      if (isHorizontal()) {
        ...
        s.x = static_cast<float>(
          static_cast<int>(_size.x * children[0]->size.x
        ...
        s.y = static_cast<float>(
          static_cast<int>(_size.y * children[0]->size.y
  ...
  Dock& getDock(const char* label, bool opened)
  {
    ...
    Dock* new_dock = static_cast<Dock*>(MemAlloc(sizeof(Dock)));
                 ...
                 bool on_border)
  {
    ...
    for (int i = 0; i < (on_border ? 4 : 5); ++i) {
      ImRect r = on_border ?
                   getSlotRectOnBorder(rect, static_cast<ImGuiDockSlot>(i)) :
                   getSlotRect(rect, static_cast<ImGuiDockSlot>(i));
      ...
      if (!IsMouseDown(0)) {
        doDock(dock, dest_dock ? dest_dock : getRootDock(),
               static_cast<ImGuiDockSlot>(i));
        return true;
      }
      ImRect docked_rect = getDockedRect(rect, static_cast<ImGuiDockSlot>(i));
  ...
  void handleDrag(Dock& dock)
  {
    ...
    Begin("##Overlay", nullptr, ImVec2(0, 0), 0.f,
  ...
  bool tabbar(Dock& dock, bool close_button)
  {
    ...
    char tmp[20];
    ImFormatString(tmp, IM_ARRAYSIZE(tmp), "tabs%d", static_cast<int>(dock.id));
  ...
  void doDock(Dock& dock, Dock* dest, ImGuiDockSlot dock_slot)
  {
    ...
    if (!dest) {
      ...
      Dock* container = static_cast<Dock*>(MemAlloc(sizeof(Dock)));
  ...
  bool begin(const char* label, bool* opened, ImGuiWindowFlags extra_flags)
  {
    ...
    if (is_float) {
      ...
      bool ret
        = Begin(label, opened, dock.size, -1.0f,
                ImGuiWindowFlags_NoCollapse /*| ImGuiWindowFlags_ShowBorders*/
                  | extra_flags); // ImGuiWindowFlags_ShowBorders not used in
                                  // new version of ImGui
      ...
      if (g.ActiveId == GetCurrentWindow()->GetID("#MOVE")
          && g.IO.MouseDown[0]) {
    ...
    if (Begin("Dock Debug Info")) {
      for (int i = 0; i < m_docks.size(); ++i) {
        if (TreeNode(reinterpret_cast<void*>(i), "Dock %d (%p)", i,
                     static_cast<void*>(m_docks[i]))) {
          Dock& dock = *m_docks[i];
          Text("pos=(%.1f %.1f) size=(%.1f %.1f)",
               static_cast<double>(dock.pos.x), static_cast<double>(dock.pos.y),
               static_cast<double>(dock.size.x),
               static_cast<double>(dock.size.y));
          Text("parent = %p\n", static_cast<void*>(dock.parent));
                      ...
                      const char* name)
{
  ...
  // specific size of docks
    ...
    for (int i = 0; i < dockSize; i++) {
      DockContext::Dock* new_dock
        = static_cast<DockContext::Dock*>(MemAlloc(sizeof(DockContext::Dock)));
  ...
  // specific index of dock
    ...
    if (g_docklist.find(context_panel.c_str()) != g_docklist.end()) {
      ...
      if (index >= 0 && index < static_cast<int>(context.m_docks.size())) {
  ...
  return static_cast<void*>(&rhelper);
}

static void readLine(ImGuiContext* /*ctx*/, ImGuiSettingsHandler* /*handler*/,
                     void* entry, const char* line_start)
{
  readHelper* userdata = static_cast<readHelper*>(entry);
  ...
  if (userdata) {
    ...
    if (sscanf(line_start, "label=%[^\n^\r]", label) == 1) {
      ...
      userdata->dock->children[0]
        = getDockByIndex(*(userdata->context), child0);
    }
    else if (sscanf(line_start, "child1=%d", &child1) == 1) {
      userdata->dock->children[1]
        = getDockByIndex(*(userdata->context), child1);
    }
                     ...
                     ImGuiTextBuffer* buf)
{
  ...
  buf->reserve(buf->size()
               + totalDockNum * static_cast<int>(sizeof(DockContext::Dock))
               + 32 * (totalDockNum + static_cast<int>(g_docklist.size()) * 2));
  ...
  for (const auto& iter : g_docklist) {
    ...
    buf->appendf("[%s][Size:%d]\n", handler->TypeName,
                 static_cast<int>(context.m_docks.size()));
    ...
    for (int i = 0, docksize = context.m_docks.size(); i < docksize; i++) {
      ...
      buf->appendf("size_x=%d\n", static_cast<int>(d->size.x));
      buf->appendf("size_y=%d\n", static_cast<int>(d->size.y));
      buf->appendf("active=%d\n", static_cast<int>(d->active));
      buf->appendf("opened=%d\n", static_cast<int>(d->opened));
      buf->appendf("location=%s\n", d->location);
      buf->appendf("status=%d\n", static_cast<int>(d->status));
      buf->appendf("prev=%d\n",
                   static_cast<int>(getDockIndex(context, d->prev_tab)));
      buf->appendf("next=%d\n",
                   static_cast<int>(getDockIndex(context, d->next_tab)));
      buf->appendf("child0=%d\n",
                   static_cast<int>(getDockIndex(context, d->children[0])));
      buf->appendf("child1=%d\n",
                   static_cast<int>(getDockIndex(context, d->children[1])));
      buf->appendf("parent=%d\n",
                   static_cast<int>(getDockIndex(context, d->parent)));
    }
  }
}
...
void ImGui::ShutdownDock()
{
  for (auto& iter : g_docklist) {
    ...
    for (int k = 0, dock_count = static_cast<int>(context.m_docks.size());
         k < dock_count; k++) {
...
bool ImGui::BeginDock(const char* label, bool* opened,
