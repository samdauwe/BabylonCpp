#ifndef BABYLON_SAMPLES_MATERIALS_SHADER_MATERIAL_PLANET_SHADERTOY_SCENE_H
#define BABYLON_SAMPLES_MATERIALS_SHADER_MATERIAL_PLANET_SHADERTOY_SCENE_H
...
public:
  ...
  static constexpr const char* customFragmentShader
    = "#ifdef GL_ES\n"
      ...
      "    float n\n"
      "      = (atmosphericOptic(p, v) + atmosphericOptic(v, u)) * (PI * 4.0);\n"
      "    float dens = atmosphericDensity(v);\n"
      "\n"
      "    float m = MAX;\n"
      "    sum\n"
      "      += dens\n"
      "         * exp(-n * (ATMOSPHERE_K_R * ATMOSPHERE_C_R + ATMOSPHERE_K_M))\n"
      ...
      "  return vec4(\n"
      "    sum\n"
      "      * (ATMOSPHERE_K_R * ATMOSPHERE_C_R * atmosphericPhaseReyleigh(cc)\n"
      "         + ATMOSPHERE_K_M * atmosphericPhaseMie(ATMOSPHERE_G_M, c, cc))\n"
      "      * ATMOSPHERE_E,\n"
      ...
      "  }\n"
      "  else {\n"
      "    if (iSphere(ro, rd, vec4(0, 0, 0, EARTH_RADIUS + EARTH_ATMOSPHERE))\n"
      "        < 0.)\n"
      "      return vec4(0.);\n"
      "\n"
      "    if (e.x > e.y) {\n"
      ...
      "  float d3 = 200.0;\n"
      "  float s1 = clamp(\n"
      "    1.0 * (h1 + rd.y * d1 - terrainMed(ro.xz + d1 * rd.xz)), 0.0, 1.0);\n"
      "  float s2 = clamp(\n"
      "    0.5 * (h1 + rd.y * d2 - terrainMed(ro.xz + d2 * rd.xz)), 0.0, 1.0);\n"
      "  float s3 = clamp(\n"
      "    0.2 * (h2 + rd.y * d3 - terrainLow(ro.xz + d3 * rd.xz)), 0.0, 1.0);\n"
      "\n"
      ...
      "       j3\n"
      "       = vec3(j0.x + sin(a) * 0.1, max(-0.25 + ca * 0.1, -0.25), j0.z);\n"
      "  float dL\n"
      ...
      "//-----------------------------------------------------\n"
      "\n"
      "vec4 renderClouds(const in vec3 ro, const in vec3 rd, const in float d,\n"
      ...
      "//-----------------------------------------------------\n"
      "\n"
      "vec4 renderPlanet(const in vec3 ro, const in vec3 rd, const in vec3 up,\n"
      "                  inout float maxd)\n"
      "{\n"
      ...
      "      vec3 finalLand = mix(mixedLand, vec3(7.0, 7.0, 7.0) * land1 * 1.5,\n"
      "                           max(iceFactor + .02 * land2 - .02, 0.));\n"
      "\n"
      "      col = (diffuse(n, SUN_DIRECTION) * sunColor + upColor) * finalLand\n"
      "            * .75;\n"
      "#ifdef HIGH_QUALITY\n"
      ...
      "    if (d > 0.) {\n"
      "      vec4 llamel = renderLlamel(rom + rdm * d, rdm, sunColor, upColor,\n"
      "                                 reflColor);\n"
      ...
      "  camint(ta, t, 7., vec3(EARTH_RADIUS + 0., EARTH_RADIUS - 500., 500.),\n"
      "         pta, dta);\n"
      ...
      "                            ro, vec3(-SUN_DIRECTION.x, SUN_DIRECTION.y,\n"
      "                                     -SUN_DIRECTION.z));\n"
      "\n"
      ...
      "          clouds.w *= 1. - smoothstep(0.8 * EARTH_CLOUDS, EARTH_CLOUDS,\n"
      "                                      lro - EARTH_RADIUS);\n"
      ...
      "                * clamp(1. - 4. * abs(offset) / EARTH_CLOUDS, 0., 1.));\n"
      "    }\n"
      "#endif\n"
      "\n"
      "    // post processing\n"
      ...
      "    col *= 0.5\n"
      "           + 0.5 * pow(16.0 * uv.x * uv.y * (1.0 - uv.x) * (1.0 - uv.y),\n"
      "                       0.1);\n"
      "  }\n"
...
#endif // end of
       // BABYLON_SAMPLES_MATERIALS_SHADER_MATERIAL_PLANET_SHADERTOY_SCENE_H
