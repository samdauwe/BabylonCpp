...
public:
  ...
  ~FireMaterialScene() override = default;
  ...
  {
    // Camera
    auto camera = ArcRotateCamera::New("Camera", 3.f * Math::PI_2, Math::PI_4, 30.f,
                                       Vector3(0.f, 4.f, 0.f), scene);
    camera->attachControl(canvas, true);

    // Light
    auto light = SpotLight::New("light", Vector3(8.f, 16.f, 8.f), Vector3(-1.f, -2.f, -1.f), 3.f,
                                1.f, scene);
    ...
    _fireMaterial->diffuseTexture    = Texture::New("textures/fire/diffuse.png", scene);
    _fireMaterial->distortionTexture = Texture::New("textures/fire/distortion.png", scene);
      ...
      [this, scene](const std::vector<AbstractMeshPtr>& meshes,
        ...
        for (const auto& mesh : meshes) {
          if (_renderWithShadows && mesh != plane) {
            _shadowGenerator->getShadowMap()->renderList().emplace_back(mesh.get());
