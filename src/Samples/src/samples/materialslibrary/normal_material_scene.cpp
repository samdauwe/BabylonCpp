...
struct NormalMaterialScene : public IRenderableScene {
  ...
  NormalMaterialScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~NormalMaterialScene() override = default;
  ...
  {
    auto camera = ArcRotateCamera::New("camera1", 0.f, 0.f, 30.f, Vector3::Zero(), scene);
    ...
    auto normalMaterial = MaterialsLibrary::NormalMaterial::New("normalMat", scene);
