...
public:
  ...
  ~SimultaneousLightsScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    scene->onBeforeRenderObservable.add([this](Scene*, EventState&) {
      _light0->position = Vector3(10.f * std::sin(_alpha), 0.f, 10.f * std::cos(_alpha));
      _light1->position = Vector3(10.f * std::sin(_alpha), 0.f, -10.f * std::cos(_alpha));
      _light2->position = Vector3(10.f * std::cos(_alpha), 0.f, 10.f * std::sin(_alpha));
...
BABYLON_REGISTER_SAMPLE("Ligths", SimultaneousLightsScene)
