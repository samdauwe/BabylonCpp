...
public:
  ...
  ~LightProjectedTextureScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    // Spot light
    _spotLight = SpotLight::New("spot02", Vector3(30.f, 40.f, 30.f), Vector3(-1.f, -2.f, -1.f),
                                1.1f, 16.f, scene);
    ...
    auto diffuseTexture    = std::static_pointer_cast<Texture>(groundMaterial->diffuseTexture());
    ...
    scene->registerBeforeRender([&](Scene* /*scene*/, EventState& /*es*/) {
      _spotLight->position = Vector3(std::cos(_alpha) * 60.f, 40.f, std::sin(_alpha) * 60.f);
      _spotLight->setDirectionToTarget(Vector3::Zero());
      _alpha += 0.01f;
    });
  }
