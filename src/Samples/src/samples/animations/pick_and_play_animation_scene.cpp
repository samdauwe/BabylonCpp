...
public:
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    for (unsigned i = 1u; i <= N; ++i) {
      ...
      auto mesh          = MeshBuilder::CreateTorusKnot("mesh" + istr, torusKnotOptions, scene);
    ...
    _animY = [this](Scene*, EventState&) {
      if (_animYEnabled) {
        ...
        for (const auto& mesh : _mesh0->getChildMeshes()) {
          mesh->position().y = 3.f * std::sin((_mesh0Offsets[i] + _t) * 2.f * Math::PI);
          ++i;
        }
        _t += 0.001f;
      }
    ...
    auto light1       = SpotLight::New("light1", Vector3(-10.f, 20.f, 0.f), Vector3(1.f, -2.f, 0.f),
                                 Math::PI_2, 1.f, scene);
    ...
    auto camera0 = ArcRotateCamera::New("camera0", Math::PI / 180.f * -140.f,
                                        Math::PI / 180.f * 90.f, 12.f, _mesh0->position(), scene);
