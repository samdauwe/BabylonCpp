...
CrowdSimulationScene05::CrowdSimulationScene05(ICanvas* iCanvas)
    : IRenderableScene(iCanvas), _crowdSimulation{nullptr}
{
}

CrowdSimulationScene05::~CrowdSimulationScene05()
{
}

const char* CrowdSimulationScene05::getName()
{
  return "Crowd Simulation Scene 05";
}

void CrowdSimulationScene05::initializeScene(ICanvas* canvas, Scene* scene)
{
...
void CrowdSimulationScene05::setupScenario(Scene* scene)
{
  // Create agent meshes
  ...
  for (unsigned int i = 0; i < 7; ++i) {
    for (unsigned int j = 0; j < 7; ++j) {
      agentId = _crowdSimulation->addAgent(
        _agentMeshes[index++], Vector3(55.f + i * 10.f, 0, 55.f + j * 10.f));
      _crowdSimulation->setAgentGoal(
        agentId, Vector2(-55.f + i * -10.f, -55.f + j * -10.f));
      ...
      _crowdSimulation->setAgentGoal(agentId,
                                     Vector2(55.f + i * 10.f, 55.f + j * 10.f));
    }
  }
