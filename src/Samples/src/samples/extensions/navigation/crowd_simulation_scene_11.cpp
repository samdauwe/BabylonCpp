...
CrowdSimulationScene11::CrowdSimulationScene11(ICanvas* iCanvas)
    : IRenderableScene(iCanvas), _crowdSimulation{nullptr}
{
}

CrowdSimulationScene11::~CrowdSimulationScene11()
{
}

const char* CrowdSimulationScene11::getName()
{
  return "Crowd Simulation Scene 11";
}

void CrowdSimulationScene11::initializeScene(ICanvas* canvas, Scene* scene)
{
...
void CrowdSimulationScene11::setupScenario(Scene* scene)
{
  // Create agent meshes
  ...
  for (unsigned int i = 0; i < 16; ++i) {
    for (unsigned int j = 0; j < 16; ++j) {
      agentId = _crowdSimulation->addAgent(
        _agentMeshes[index++], Vector3(205.f + i * 1.f, 0.f, 0.f + j * 1.f));
