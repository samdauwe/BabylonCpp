...
BuildingInstancesGridScene::BuildingInstancesGridScene(ICanvas* iCanvas)
    : IRenderableScene(iCanvas), _sun{nullptr}, _light2{nullptr}, _alpha{0.f}
{
}

BuildingInstancesGridScene::~BuildingInstancesGridScene()
{
}

const char* BuildingInstancesGridScene::getName()
{
  return "Building Instances Grid Scene";
}

void BuildingInstancesGridScene::initializeScene(ICanvas* canvas, Scene* scene)
{
  // Adding a light
  auto light     = PointLight::New("Omni", Vector3(20.f, 100.f, 20.f), scene);
  light->diffuse = Color3(0.1f, 0.1f, 0.17f);
  _light2
    = DirectionalLight::New("dirlight", Vector3(1.f, -0.75f, 0.25f), scene);
  _light2->diffuse = Color3(0.95f, 0.7f, 0.4f);

  // Adding an Arc Rotate Camera
  auto camera
    = ArcRotateCamera::New("Camera", 0.2f, 1.27f, 45.f, Vector3::Zero(), scene);
  ...
  for (float i = 0; i < grid_size; ++i) {
    for (float j = 0; j < grid_size; ++j) {
      ...
      float diff = std::min(std::abs(j - grid_size / 2.f),
                            std::abs(i - grid_size / 2.f));
      if (diff < 2.f) {
        continue;
      }
      ...
      inst->scaling().y  = size;
      inst->scaling().x
        = grid_spacing * 0.5f + Math::random() * grid_spacing * 0.4f;
      inst->scaling().z  = inst->scaling().x;
      inst->position().x = -grid_size * 0.425f * grid_spacing
                           + i * grid_spacing * 0.85f
                           + Math::random() * 0.15f * grid_spacing;
      inst->position().z = -grid_size * 0.425f * grid_spacing
                           + j * grid_spacing * 0.85f
                           + Math::random() * 0.15f * grid_spacing;
    }
  }
  ...
  scene->registerBeforeRender([this](Scene* /*scene*/, EventState& /*es*/) {
    ...
    auto color
      = Color3::Lerp(Color3(1.f, 1.f, 0.5f), Color3(1.f, 0.f, 0.f), 1.f + alt);
