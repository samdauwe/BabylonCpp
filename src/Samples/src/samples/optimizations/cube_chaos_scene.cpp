...
CubeChaosScene::CubeChaosScene(ICanvas* iCanvas)
...
CubeChaosScene::~CubeChaosScene()
{
}

const char* CubeChaosScene::getName()
{
  return "Cube Chaos Scene";
}

void CubeChaosScene::initializeScene(ICanvas* /*canvas*/, Scene* scene)
{
  ...
  auto mainLight
    = DirectionalLight::New("MainLight", Vector3(-1.f, -1.f, 1.f), scene);
  mainLight->intensity = 0.7f;

  auto cameraLight
    = PointLight::New("cameraLight", Vector3(1.f, 10.f, 1.f), scene);
