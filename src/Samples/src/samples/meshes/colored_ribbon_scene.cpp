...
struct ColoredRibbonScene : public IRenderableScene {
  ...
  ColoredRibbonScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~ColoredRibbonScene() override = default;
    ...
    for (auto l = 0; l < mapSubZ; l++) {
      ...
      for (auto w = 0; w < mapSubX; w++) {
        ...
        auto y = static_cast<float>(
          noise.noise(static_cast<double>(x * noiseScale), static_cast<double>(z * noiseScale)));
