...
public:
  ...
  ~HouseFromFloorplanScene() override = default;
  ...
  {
    // Create a camera
    auto camera = ArcRotateCamera::New("Camera", -Math::PI_2, Math::PI / 3.f, 25.f,
                                       Vector3(0.f, 0.f, 4.5f), scene);
    camera->setPosition(Vector3(0, 5, 12));
    camera->attachControl(canvas, true);

    // Create a light
    auto light       = HemisphericLight::New("hemiLight", Vector3(5.f, 10.f, 0.f), scene);
...
private:
                        ...
                        BuildFromPlanOptions& options, Scene* scene)
  {
    ...
    for (size_t w = 0; w < nbWalls; ++w) {
      angle
        = Math::PI - std::acos(Vector3::Dot(line, nextLine) / (line.length() * nextLine.length()));
      direction       = Vector3::Cross(nextLine, line).normalize().y;
      auto lineNormal = Vector3(line.z, 0.f, -1.f * line.x).normalize();
      line.normalize();
      outerData[(w + 1) % nbWalls] = walls[(w + 1) % nbWalls]
    ...
    for (size_t w = 0; w < nbWalls; w++) {
      innerTopCorners.emplace_back(Vector3(innerBaseCorners[w].x, height, innerBaseCorners[w].z));
      outerTopCorners.emplace_back(Vector3(outerBaseCorners[w].x, height, outerBaseCorners[w].z));
    }

    auto maxL = 0.f;
    for (size_t w = 0; w < nbWalls; ++w) {
      maxL = std::max(innerBaseCorners[(w + 1) % nbWalls].subtract(innerBaseCorners[w]).length(),
                      maxL);
    }
    ...
    const auto compareLeft = [](const DoorSpace& a, const DoorSpace& b) { return a.left - b.left; };
    ...
    for (size_t w = 0; w < nbWalls; w++) {
      ...
      exteriorWallLength
        = outerBaseCorners[(w + 1) % nbWalls].subtract(outerBaseCorners[w]).length();
      ...
      for (size_t d = 0; d < doors; d++) {
        ...
        polygonCorners.emplace_back(
          Vector2(walls[w].doorSpaces[d].left, walls[w].doorSpaces[d].door.height));
        polygonCorners.emplace_back(
          Vector2(walls[w].doorSpaces[d].left + walls[w].doorSpaces[d].door.width,
                  walls[w].doorSpaces[d].door.height));
        polygonCorners.emplace_back(
          Vector2(walls[w].doorSpaces[d].left + walls[w].doorSpaces[d].door.width, 0.f));
      }
      ...
      PolygonMeshBuilder polygonTriangulation = PolygonMeshBuilder("", polygonCorners, scene);
      ...
      for (size_t ws = 0; ws < windows; ++ws) {
        ...
        holeData.emplace_back<Vector2>(
          Vector2(walls[w].windowSpaces[ws].left, height - walls[w].windowSpaces[ws].top
        ...
        holeData.emplace_back<Vector2>(Vector2(
          ...
          height - walls[w].windowSpaces[ws].top - walls[w].windowSpaces[ws].window.height));
        holeData.emplace_back<Vector2>(
          Vector2(walls[w].windowSpaces[ws].left + walls[w].windowSpaces[ws].window.width,
      ...
      std::tie(wallData.positions, wallData.indices)
        = polygonTriangulation.buildWall(walls[w].corner, walls[(w + 1) % nbWalls].corner);
      ...
      // has 3 data points
      windowData = stl_util::slice(wallData.positions, static_cast<int>(12 * (doors + 1)));
      doorData   = stl_util::slice(wallData.positions, 3, static_cast<int>(3 * (4 * doors + 1)));
      ...
      for (size_t p = 0; p < doorData.size() / 12; ++p) {
        ...
        for (size_t d = 0; d < 4; ++d) {
          doorsIn.emplace_back(Vector3(doorData[3 * d + 12 * p], doorData[3 * d + 12 * p + 1],
                                       doorData[3 * d + 12 * p + 2]));
          doorData[3 * d + 12 * p] += ply * wallNormal.x;
          doorData[3 * d + 12 * p + 2] += ply * wallNormal.z;
          doorsOut.emplace_back(Vector3(doorData[3 * d + 12 * p], doorData[3 * d + 12 * p + 1],
                                        doorData[3 * d + 12 * p + 2]));
        }
      ...
      for (size_t p = 0; p < windowData.size() / 12; ++p) {
        ...
        for (size_t d = 0; d < 4; ++d) {
          windowsIn.emplace_back(Vector3(windowData[3 * d + 12 * p], windowData[3 * d + 12 * p + 1],
      ...
      stl_util::concat(wallData.positions, {outerBaseCorners[(w + 1) % nbWalls].x,
                                            outerBaseCorners[(w + 1) % nbWalls].y,
                                            outerBaseCorners[(w + 1) % nbWalls].z});
      stl_util::concat(wallData.positions,
                       {outerTopCorners[(w + 1) % nbWalls].x, outerTopCorners[(w + 1) % nbWalls].y,
      ...
      for (const auto& p : polyTriPointsElements) {
        if (p.x == 0.f) {
          ...
          uvx = exteriorUV.x
                + (0.5f * wallDiff + p.x) * (exteriorUV.z - exteriorUV.x) / (maxL + wallDiff);
        }
      ...
      if (doorsRemaining > 0) {
        ...
        stl_util::concat(
          positions, {innerBaseCorners[w].x, innerBaseCorners[w].y, innerBaseCorners[w].z}); // tl
        stl_util::concat(
          positions, {outerBaseCorners[w].x, outerBaseCorners[w].y, outerBaseCorners[w].z}); // bl
        stl_util::concat(positions,
                         {innerDoorCorners[w][doorNb][0].x, innerDoorCorners[w][doorNb][0].y,
                          innerDoorCorners[w][doorNb][0].z}); // tr
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][0].x, outerDoorCorners[w][doorNb][0].y,
                          outerDoorCorners[w][doorNb][0].z}); // br

        stl_util::concat(
          uvs,
          {exteriorUV.x, exteriorUV.y + (exteriorUV.w - exteriorUV.y) * ply / maxH}); // top Left
        stl_util::concat(uvs, {exteriorUV.x, exteriorUV.y});                          // base Left
        stl_util::concat(
          uvs,
          {exteriorUV.x + (exteriorUV.z - exteriorUV.x) * walls[w].doorSpaces[doorNb].left / maxL,
        ...
        stl_util::concat(indices, {_nbIndices, _nbIndices + 2, _nbIndices + 3, _nbIndices + 3,
                                   _nbIndices + 1, _nbIndices});
        ...
        stl_util::concat(positions,
                         {innerDoorCorners[w][doorNb][1].x, innerDoorCorners[w][doorNb][1].y,
                          innerDoorCorners[w][doorNb][1].z}); // tr
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][0].x, outerDoorCorners[w][doorNb][0].y,
                          outerDoorCorners[w][doorNb][0].z}); // bl
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][1].x, outerDoorCorners[w][doorNb][1].y,
                          outerDoorCorners[w][doorNb][1].z}); // tl

        stl_util::concat(uvs, {exteriorUV.x + (exteriorUV.z - exteriorUV.x) * ply / maxL,
                               exteriorUV.y}); // base right
        stl_util::concat(uvs,
        ...
        stl_util::concat(indices, {_nbIndices, _nbIndices + 1, _nbIndices + 3, _nbIndices,
        ...
        stl_util::concat(positions,
                         {innerDoorCorners[w][doorNb][2].x, innerDoorCorners[w][doorNb][2].y,
                          innerDoorCorners[w][doorNb][2].z}); // br
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][1].x, outerDoorCorners[w][doorNb][1].y,
                          outerDoorCorners[w][doorNb][1].z}); // tl
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][2].x, outerDoorCorners[w][doorNb][2].y,
                          outerDoorCorners[w][doorNb][2].z}); // tr

        stl_util::concat(uvs, {exteriorUV.x, exteriorUV.y}); // base Left
        stl_util::concat(
          uvs, {exteriorUV.x
                  + (exteriorUV.z - exteriorUV.x) * walls[w].doorSpaces[doorNb].door.width / maxL,
                exteriorUV.y}); // base right
        stl_util::concat(
          uvs,
          {exteriorUV.x, exteriorUV.y + (exteriorUV.w - exteriorUV.y) * ply / maxH}); // top Left
        ...
        stl_util::concat(indices, {_nbIndices + 2, _nbIndices + 1, _nbIndices + 3, _nbIndices + 2,
                                   _nbIndices, _nbIndices + 1});
        ...
        stl_util::concat(positions,
                         {innerDoorCorners[w][doorNb][3].x, innerDoorCorners[w][doorNb][3].y,
                          innerDoorCorners[w][doorNb][3].z}); // bl
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][2].x, outerDoorCorners[w][doorNb][2].y,
                          outerDoorCorners[w][doorNb][2].z}); // tr
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][3].x, outerDoorCorners[w][doorNb][3].y,
        ...
        stl_util::concat(indices, {_nbIndices, _nbIndices + 3, _nbIndices + 2, _nbIndices,
      ...
      while (doorsRemaining > 0) {
        ...
        stl_util::concat(indices, {_nbIndices, _nbIndices + 1, _nbIndices + 3, _nbIndices + 3,
                                   _nbIndices + 2, _nbIndices});
      ...
      if (doors > 0) {
        stl_util::concat(positions,
                         {innerDoorCorners[w][doorNb][3].x, innerDoorCorners[w][doorNb][3].y,
                          innerDoorCorners[w][doorNb][3].z}); // bl
        stl_util::concat(positions, {innerBaseCorners[(w + 1) % nbWalls].x,
                                     innerBaseCorners[(w + 1) % nbWalls].y,
                                     innerBaseCorners[(w + 1) % nbWalls].z}); // br
        stl_util::concat(positions,
                         {outerDoorCorners[w][doorNb][3].x, outerDoorCorners[w][doorNb][3].y,
                          outerDoorCorners[w][doorNb][3].z}); // tl
        stl_util::concat(positions, {outerBaseCorners[(w + 1) % nbWalls].x,
                                     outerBaseCorners[(w + 1) % nbWalls].y,
        ...
        stl_util::concat(
          ...
          {exteriorUV.x
             + (exteriorUV.z - exteriorUV.x)
                 * (wallLength
                    - (walls[w].doorSpaces[doorNb].left + walls[w].doorSpaces[doorNb].door.width))
                 / maxL,
           exteriorUV.y + (exteriorUV.w - exteriorUV.y) * ply / maxH}); // top right
      }
      else {
        stl_util::concat(
          positions, {innerBaseCorners[w].x, innerBaseCorners[w].y, innerBaseCorners[w].z}); // bl
        stl_util::concat(positions, {innerBaseCorners[(w + 1) % nbWalls].x,
                                     innerBaseCorners[(w + 1) % nbWalls].y,
                                     innerBaseCorners[(w + 1) % nbWalls].z}); // br
        stl_util::concat(
          positions, {outerBaseCorners[w].x, outerBaseCorners[w].y, outerBaseCorners[w].z}); // tl
        stl_util::concat(positions, {outerBaseCorners[(w + 1) % nbWalls].x,
                                     outerBaseCorners[(w + 1) % nbWalls].y,
                                     outerBaseCorners[(w + 1) % nbWalls].z}); // tr

        stl_util::concat(uvs, {exteriorUV.x, exteriorUV.y}); // base Left
        stl_util::concat(uvs, {exteriorUV.x + (exteriorUV.z - exteriorUV.x) * wallLength / maxL,
                               exteriorUV.y}); // base right
        stl_util::concat(
          uvs,
          {exteriorUV.x, exteriorUV.y + (exteriorUV.w - exteriorUV.y) * ply / maxH}); // top Left
      ...
      for (size_t ww = 0; ww < innerWindowCorners[w].size(); ++ww) {
        ...
        stl_util::concat(positions, {innerWindowCorners[w][ww][3].x, innerWindowCorners[w][ww][3].y,
                                     innerWindowCorners[w][ww][3].z}); // tr
        stl_util::concat(positions, {innerWindowCorners[w][ww][0].x, innerWindowCorners[w][ww][0].y,
                                     innerWindowCorners[w][ww][0].z}); // br
        stl_util::concat(positions, {outerWindowCorners[w][ww][3].x, outerWindowCorners[w][ww][3].y,
                                     outerWindowCorners[w][ww][3].z}); // tl
        stl_util::concat(positions, {outerWindowCorners[w][ww][0].x, outerWindowCorners[w][ww][0].y,
        ...
        stl_util::concat(indices, {_nbIndices + 1, _nbIndices, _nbIndices + 3, _nbIndices + 2,
                                   _nbIndices + 3, _nbIndices});
        ...
        stl_util::concat(positions, {innerWindowCorners[w][ww][1].x, innerWindowCorners[w][ww][1].y,
                                     innerWindowCorners[w][ww][1].z}); // tr
        stl_util::concat(positions, {outerWindowCorners[w][ww][0].x, outerWindowCorners[w][ww][0].y,
                                     outerWindowCorners[w][ww][0].z}); // bl
        stl_util::concat(positions, {outerWindowCorners[w][ww][1].x, outerWindowCorners[w][ww][1].y,
        ...
        stl_util::concat(
          uvs, {exteriorUV.x
                  + (exteriorUV.z - exteriorUV.x) * walls[w].windowSpaces[ww].window.width / maxL,
                exteriorUV.y + (exteriorUV.w - exteriorUV.y) * ply / maxH}); // top right
        stl_util::concat(uvs, {exteriorUV.x, exteriorUV.y});                 // base Left
        stl_util::concat(
          uvs, {exteriorUV.x
        ...
        stl_util::concat(indices, {_nbIndices + 1, _nbIndices, _nbIndices + 3, _nbIndices + 3,
                                   _nbIndices, _nbIndices + 2});
        ...
        stl_util::concat(positions, {innerWindowCorners[w][ww][2].x, innerWindowCorners[w][ww][2].y,
                                     innerWindowCorners[w][ww][2].z}); // tl
        stl_util::concat(positions, {outerWindowCorners[w][ww][1].x, outerWindowCorners[w][ww][1].y,
                                     outerWindowCorners[w][ww][1].z}); // br
        stl_util::concat(positions, {outerWindowCorners[w][ww][2].x, outerWindowCorners[w][ww][2].y,
        ...
        stl_util::concat(indices, {_nbIndices + 1, _nbIndices + 2, _nbIndices + 3, _nbIndices,
                                   _nbIndices + 2, _nbIndices + 1});
        ...
        stl_util::concat(indices, {_nbIndices + 3, _nbIndices, _nbIndices + 2, _nbIndices + 1,
                                   _nbIndices, _nbIndices + 3});
      }
      ...
      stl_util::concat(positions,
                       {innerTopCorners[(w + 1) % nbWalls].x, innerTopCorners[(w + 1) % nbWalls].y,
      ...
      stl_util::concat(
        uvs, {uvx, exteriorUV.y + (exteriorUV.w - exteriorUV.y) * ply / maxH}); // top Left

      uvx = exteriorUV.x + (0.5f * wallDiff + wallLength) * (exteriorUV.z - exteriorUV.x) / maxL;
      stl_util::concat(
        uvs, {uvx, exteriorUV.y + (exteriorUV.w - exteriorUV.y) * ply / maxH}); // top right

      stl_util::concat(uvs, {exteriorUV.x, exteriorUV.y}); // base Left
      stl_util::concat(
        uvs, {exteriorUV.x + (exteriorUV.z - exteriorUV.x) * exteriorWallLength / (maxL + wallDiff),
              exteriorUV.y}); // base right
