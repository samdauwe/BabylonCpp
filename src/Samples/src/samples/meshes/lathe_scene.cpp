...
struct LatheScene : public IRenderableScene {
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    for (auto& point : points) {
      auto sphere      = Mesh::CreateSphere("sphere" + std::to_string(i++), 8, 0.15f, scene, true);
    ...
    auto createSegment = [&points](unsigned int _i) {
      return Curve3::CreateHermiteSpline(points[_i][0], points[_i][1], points[_i + 1][0],
                                         points[_i + 1][1], 32);
    };
    ...
    auto tube = Mesh::CreateTube("tube", spline.getPoints(), 0.025f, 16, radius, 1, scene, false);
    tube->position = Vector3(0.0125f, 0.f, 0.f);

    // Create a lathe from the spline
    auto lathe = Mesh::CreateLathe("tube", spline.getPoints(), 1, 16, scene, false);
