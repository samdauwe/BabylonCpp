...
struct ColoredTriangleScene : public IRenderableScene {
  ...
  ColoredTriangleScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~ColoredTriangleScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    auto light      = HemisphericLight::New("HemiLight", Vector3(-2.f, 0.f, -20.f), scene);
    light->specular = Color3(0.5f, 0.5f, 0.5f);

    // https://doc.babylonjs.com/babylon101/cameras
    auto camera      = ArcRotateCamera::New("camera1", -Math::PI_2 + 0.5f, Math::PI_2, Math::PI,
                                       Vector3::Zero(), scene);
