...
public:
  SuperEllipsoidScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~SuperEllipsoidScene() override = default;
...
private:
                           ...
                           float scaleZ) const
  {
    ...
    float sinPhi  = std::sin(phi);
    float sinBeta = std::sin(beta);
    normal.x = Scalar::Signf(cosPhi) * std::pow(std::abs(cosPhi), 2 - n1) * Scalar::Signf(cosBeta)
               * std::pow(std::abs(cosBeta), 2 - n2) / scaleX;
    normal.z = Scalar::Signf(cosPhi) * std::pow(std::abs(cosPhi), 2 - n1) * Scalar::Signf(sinBeta)
               * std::pow(std::abs(sinBeta), 2 - n2) / scaleY;
    normal.y = Scalar::Signf(sinPhi) * std::pow(std::abs(sinPhi), 2 - n1) / scaleZ;
    normal.normalize();
    return normal;
  }

  MeshPtr _createSuperEllipsoid(size_t samples, float n1, float n2, float scalex, float scaley,
                                float scalez, Scene* scene)
  {
    ...
    for (size_t j = 0; j <= samples / 2; j++) {
      ...
      for (size_t i = 0; i <= samples; i++) {
        // Triangle #1
        vertices.emplace_back(_sampleSuperEllipsoid(phi, beta, n1, n2, scalex, scaley, scalez));
        normals.emplace_back(_calculateNormal(phi, beta, n1, n2, scalex, scaley, scalez));
        vertices.emplace_back(
          _sampleSuperEllipsoid(phi + dP, beta, n1, n2, scalex, scaley, scalez));
        normals.emplace_back(_calculateNormal(phi + dP, beta, n1, n2, scalex, scaley, scalez));
        vertices.emplace_back(
          _sampleSuperEllipsoid(phi + dP, beta + dB, n1, n2, scalex, scaley, scalez));
        normals.emplace_back(_calculateNormal(phi + dP, beta + dB, n1, n2, scalex, scaley, scalez));
        // Triangle #2
        vertices.emplace_back(_sampleSuperEllipsoid(phi, beta, n1, n2, scalex, scaley, scalez));
        normals.emplace_back(_calculateNormal(phi, beta, n1, n2, scalex, scaley, scalez));
        vertices.emplace_back(
        ...
        normals.emplace_back(_calculateNormal(phi, beta + dB, n1, n2, scalex, scaley, scalez));
        beta += dB;
      }
      phi += dP;
    }
