...
public:
  BasicElementsScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~BasicElementsScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    auto exponentialPath = [](std::vector<Vector3>& path, float p) {
      for (float i = -10.f; i < 10.f; ++i) {
        path.emplace_back(
          Vector3(p, i, std::sin(p / 3.f) * 5.f * std::exp(-(i - p) * (i - p) / 60.f) + i / 3.f));
      }
    };
    ...
    auto ribbon = Mesh::CreateRibbon("ribbon", arrayOfPaths, false, false, 0, scene);
...
BABYLON_REGISTER_SAMPLE("Meshes", BasicElementsScene)
