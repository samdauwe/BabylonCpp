...
public:
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    // Box animations
    scene->onBeforeRenderObservable.add([this](Scene* /*scene*/, EventState & /*es*/) -> void {
      ...
      for (unsigned int boxIndex = 0; boxIndex < _boxes.size(); ++boxIndex) {
        ...
        auto i = std::get<0>(boxIndices), j = std::get<1>(boxIndices), k = std::get<2>(boxIndices);
