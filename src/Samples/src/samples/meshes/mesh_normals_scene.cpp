...
struct MeshNormalsScene : public IRenderableScene {
  ...
  ~MeshNormalsScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    auto showNormals = [](const MeshPtr& mesh, float size, const Color3& color, Scene* scene) {
      ...
      for (unsigned int i = 0; i < normals.size(); i += 3) {
        ...
        std::vector<Vector3> line{v1.add(mesh->position()), v2.add(mesh->position())};
        lines.emplace_back(line);
      }
      LineSystemOptions options;
      options.lines      = std::move(lines);
      auto normalLines   = MeshBuilder::CreateLineSystem("normalLines", options, scene);
