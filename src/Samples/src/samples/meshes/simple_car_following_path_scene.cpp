...
/**
 * @brief Simple Car Following Path. The car will consist of a trapezoid for the body, four
 * cylinders for wheels. The car will be animated to follow a path an remain parallel to it. The
 * wheels will rotate and be given a texture to show that they are rotating. For simplicity all
 * wheels will remain parallel to the car and their rotation will not be matched to the forward
 * speed of the car. The car will move forward by following a sequence of points on the path. The
...
public:
  ...
  ~SimpleCarFollowingPathScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    const std::vector<Vector3> extrudePath = {Vector3(0.f, 0.f, 0.f), Vector3(0.f, 0.f, 4.f)};
    ...
    const auto createWheel
      = [this, &scene, &wheelMaterial](const std::string& wheelName, const Vector3& position) {
    ...
    for (unsigned int i = 0; i < _curveNumberOfPoints + 1; ++i) {
      _points.emplace_back(Vector3(
        (r + (r / 5.f) * std::sin(8.f * i * Math::PI / n)) * std::sin(2.f * i * Math::PI / n), 0.f,
        (r + (r / 10.f) * std::sin(6.f * i * Math::PI / n)) * std::cos(2.f * i * Math::PI / n)));
    }
