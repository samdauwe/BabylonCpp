...
class AirplaneModelScene : public IRenderableScene {
  ...
  using MakeWingFunction
    = std::function<MeshPtr(const MeshPtr& mesh, float radius, float fuselageXer, bool invertY)>;

public:
  AirplaneModelScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~AirplaneModelScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    auto spot = SpotLight::New("spot", Vector3(25.f, 15.f, -10.f), Vector3(-1.f, -0.8f, 1.f), 15.f,
                               1.f, scene);
    ...
    const auto createSphereOptions2 = [](float diameter, unsigned int segments, bool updatable,
                                         unsigned int sideOrientation) -> SphereOptions {
      SphereOptions options;
      options.diameter        = diameter;
      options.segments        = segments;
    ...
    const auto createCylinderOptions1 = [](float diameter, float diameterTop, float height,
    ...
    const auto createCylinderOptions2
      = [](float diameter, float height, unsigned int tessellation) -> CylinderOptions {
      CylinderOptions options;
      options.diameter     = diameter;
      options.height       = height;
      options.tessellation = tessellation;
    ...
    auto noseOptions     = createSphereOptions1(diameter, 0.5f, 16u, true, Mesh::FRONTSIDE);
    auto nose            = MeshBuilder::CreateSphere("nose", noseOptions, scene);
    auto noseconeOptions = createCylinderOptions1(radius * 2.02f, radius, diameter, 32);
    auto nosecone        = MeshBuilder::CreateCylinder("nosecone", noseconeOptions, scene);
    auto taperOptions    = createSphereOptions1(radius * 2.05f, 0.45f, 16u, true, Mesh::FRONTSIDE);
    auto taper           = MeshBuilder::CreateSphere("taper", taperOptions, scene);
    auto flightdeckOptions      = createSphereOptions2(diameter, 16u, true, Mesh::FRONTSIDE);
    auto flightdeck             = MeshBuilder::CreateSphere("flightdeck", flightdeckOptions, scene);
    auto flightdecktaperOptions = createCylinderOptions2(radius * 1.5f, fuselength * 2.f, 32);
    auto flightdecktaper
      = MeshBuilder::CreateCylinder("flightdecktaper", flightdecktaperOptions, scene);
    auto fuselageOptions  = createCylinderOptions2(diameter, fuselength * 2.f, 32);
    auto fuselage         = MeshBuilder::CreateCylinder("fuselage", fuselageOptions, scene);
    ...
    nose->position     = Vector3((0.16f - 1.60f - fuselageXer) * radius, -0.090f * radius, 0.f);

    nosecone->rotation().z = 133.f * Math::PI / 256.f;
    nosecone->scaling      = Vector3(1.f, 0.8f, 1.f);
    nosecone->position     = Vector3((0.16f - 0.90f - fuselageXer) * radius, -0.055f * radius, 0.f);

    flightdeck->scaling  = Vector3(2.f, 1.f, 0.8f);
    flightdeck->position = Vector3((1.04f - fuselageXer) * radius, 0.4f * radius, 0.f);

    flightdecktaper->position     = Vector3(0.2f * radius * fuselageXer, 0.36f * radius, 0.f);
    flightdecktaper->rotation().z = (128.f - (16.f / fuselageXer)) * Math::PI / 256.f;
    ...
    taper->position     = Vector3((fuselageXer - 0.85f) * radius, 0.f - 0.14f * radius, 0.f);
    ...
    auto airplane
      = Mesh::MergeMeshes({nose, nosecone, flightdeck, flightdecktaper, fuselage, taper, leftwing,
...
private:
  ...
  MakeWingFunction _makeFormAppendage(float frontBaseV, float backBaseV, float ySize, float xOrigin,
                                      float yOrigin, float baseXsize, float xSweepback)
  {
    ...
    const auto makeWing = [=](const MeshPtr& mesh, float radius, float fuselageXer, bool invertY) {
      ...
      auto wingYorigin      = (0.f - yOrigin) * radius * yMultiplier; // radius of fuselage
