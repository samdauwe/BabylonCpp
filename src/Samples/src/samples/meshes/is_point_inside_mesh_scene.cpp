...
/**
 * @brief Is Point Inside Mesh Scene. Example demonstrating how to check when a point is inside a
 * mesh.
...
struct IsPointInsideMeshScene : public IRenderableScene {
  ...
  IsPointInsideMeshScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~IsPointInsideMeshScene() override = default;
  ...
  {
    auto camera = ArcRotateCamera::New("Camera", 3.f * Math::PI_2, Math::PI / 8.f, 20.f,
                                       Vector3::Zero(), scene);
    ...
    // Outside mesh sphere
    auto sphereOutside      = MeshBuilder::CreateSphere("pointOutside", options, scene);
    ...
    for (size_t p = 0; p < 1000; ++p) {
      ...
      auto instance = (tps->pointIsInside(point)) ? sphereInside->createInstance("") :
