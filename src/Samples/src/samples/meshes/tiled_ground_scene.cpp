...
struct TiledGroundScene : public IRenderableScene {
  ...
  TiledGroundScene(ICanvas* iCanvas) : IRenderableScene(iCanvas)
  {
  }

  ~TiledGroundScene() override = default;
  ...
  void initializeScene(ICanvas* canvas, Scene* scene) override
  {
    ...
    auto camera
      = ArcRotateCamera::New("Camera", -Math::PI_2, Math::PI / 3.f, 12.f, Vector3::Zero(), scene);
    ...
    auto tiledGround = Mesh::CreateTiledGround("Tiled Ground", xmin, zmin, xmax, zmax, subdivisions,
                                               precision, scene);
    ...
    for (int row = 0; row < subdivisions.height; row++) {
      for (int col = 0; col < subdivisions.width; col++) {
        tiledGround->subMeshes.emplace_back(
          SubMesh::New(row % 2 ^ col % 2, 0, verticesCount, base, tileIndicesLength, tiledGround));
...
BABYLON_REGISTER_SAMPLE("Meshes", TiledGroundScene)
