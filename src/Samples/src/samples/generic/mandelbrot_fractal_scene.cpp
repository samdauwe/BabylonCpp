...
MandelbrotFractalScene::MandelbrotFractalScene(ICanvas* iCanvas)
    : IRenderableScene(iCanvas)
{
}

MandelbrotFractalScene::~MandelbrotFractalScene()
{
}

const char* MandelbrotFractalScene::getName()
{
  return "Mandelbrot Fractal Scene";
}

void MandelbrotFractalScene::initializeScene(ICanvas* canvas, Scene* scene)
{
  // Create a camera
  _camera = ArcRotateCamera::New("camera1", Math::PI2, 0.268f, 400.f,
                                 Vector3::Zero(), scene);
  ...
  _scene->registerBeforeRender([this]() { _pl->position = _camera->position; });
}
